...
abstract class AbstractHistogram3D extends Histogram implements IHistogram3D {
    ...
    @Serial
    private static final long serialVersionUID = -4531485479393730774L;
    ...
    public int allEntries() {
        ...
        for (int i = xAxis.bins(); --i >= -2; )
            for (int j = yAxis.bins(); --j >= -2; )
                for (int k = zAxis.bins(); --k >= -2; ) n += binEntries(i, j, k);
        return n;
    }
    ...
    public int entries() {
        ...
        for (int i = 0; i < xAxis.bins(); i++)
            for (int j = 0; j < yAxis.bins(); j++)
                for (int k = 0; k < zAxis.bins(); k++) n += binEntries(i, j, k);
        return n;
    }
    ...
    /**
     * The precise meaning of the arguments to the public slice methods is somewhat ambiguous, so we define this
     * internal slice method and clearly specify its arguments.
     * <p>
