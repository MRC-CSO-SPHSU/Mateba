...
public class DynamicFloatBin1D extends QuantileFloatBin1D {
     ...
     */
    private synchronized cern.colt.map.tfloat.AbstractFloatIntMap frequencyMap() {
        // cern.colt.map.OpenFloatIntHashMap.hashCollisions = 0;
        // fill a map that collects frequencies
        cern.colt.map.tfloat.AbstractFloatIntMap map = new cern.colt.map.tfloat.OpenFloatIntHashMap();
    ...
    public synchronized void sample(int n, boolean withReplacement, FloatRandomEngine randomGenerator,
        ...
        if (!withReplacement) { // without
            ...
            cern.jet.random.tfloat.FloatUniform uniform = new cern.jet.random.tfloat.FloatUniform(randomGenerator);
            int s = size();
            for (int i = n; --i >= 0;) {
    ...
    public synchronized DynamicFloatBin1D sampleBootstrap(DynamicFloatBin1D other, int resamples,
        ...
        cern.colt.buffer.tfloat.FloatBuffer buffer1 = sample1.buffered(Math.min(maxCapacity, s1));

        DynamicFloatBin1D sample2 = new DynamicFloatBin1D();
        cern.colt.buffer.tfloat.FloatBuffer buffer2 = sample2.buffered(Math.min(maxCapacity, s2));

        DynamicFloatBin1D bootstrap = new DynamicFloatBin1D();
        cern.colt.buffer.tfloat.FloatBuffer bootBuffer = bootstrap.buffered(Math.min(maxCapacity, resamples));
