 ...
 * truly random subset of 10^6 objects and do something with it, for example,
 * compute the sum over some instance field, or whatever. How do we choose the
 ...
 * example, if you want to compute 10^10 such numbers you can do this by
 * computing them in blocks of, say, 500 elements each. You then need only space
 * to keep one block of 500 elements (i.e. 4 KB). When you are finished
 * processing the first 500 elements you call <tt>nextBlock</tt> to fill the
 ...
 * 
 * @see FloatRandomSamplingAssistant
 * @author wolfgang.hoschek@cern.ch
 * @version 1.1 05/26/99
 */
public class FloatRandomSampler extends cern.colt.PersistentObject {
    /**
     * 
     */
    ...
    protected static void rejectMethodD(long n, long N, int count, long low, long[] values, int fromIndex,
         ...
         * sampleMethodA() and sampleMethodD(). The idea is that it is more
        ...
        float nreal, Nreal, ninv, nmin1inv, U, X, Vprime, y1, y2, top, bottom, negSreal, qu1real;
        long qu1, t, limit;
        // long threshold;
        long S;
        long chosen = -1 + low;

        long negalphainv = -13; // tuning paramter, determines when to switch
        // from method D to method A. Dependent on
        // programming language, platform, etc.

        nreal = n;
        ninv = (float) (1.0 / nreal);
        Nreal = N;
        Vprime = (float) Math.exp(Math.log(randomGenerator.raw()) * ninv);
        qu1 = -n + 1 + N;
        qu1real = -nreal + 1.0f + Nreal;
        // threshold = -negalphainv * n;

        while (n > 1 && count > 0) { // && threshold<N) {
            nmin1inv = (float) (1.0 / (-1.0 + nreal));
            for (;;) {
                ...
                // step D3: Accept?
                y1 = (float) Math.exp(Math.log(U * Nreal / qu1real) * nmin1inv);
                ...
                if (Nreal / (-X + Nreal) >= y1 * Math.exp(Math.log(y2) * nmin1inv)) {
                    // accept !
                    Vprime = (float) Math.exp(Math.log(randomGenerator.raw()) * nmin1inv);
                    break; // break inner loop
                }
                Vprime = (float) Math.exp(Math.log(randomGenerator.raw()) * ninv);
            } // end for
    ...
    protected static void sampleMethodD(long n, long N, int count, long low, long[] values, int fromIndex,
        ...
        while (n > 1 && count > 0 && threshold < N) {
            ...
            values[fromIndex++] = chosen;
            /*
             * // invert for (int iter=0; iter<S && count > 0; iter++) {
             * values[fromIndex++] = ++chosen; count--; } chosen++;
             */
            count--;
    ...
    public static void test(long n, long N, long low, int chunkSize, int times) {
        ...
        for (long t = times; --t >= 0;) {
            FloatRandomSampler sampler = new FloatRandomSampler(n, N, low,
                    cern.jet.random.tfloat.AbstractFloatDistribution.makeDefaultGenerator());
            for (long i = 0; i < chunks; i++) {
                sampler.nextBlock(chunkSize, values, 0);

                /*
                 * Log.print("Chunk #"+i+" = ["); for (int j=0; j<chunkSize-1;
                 * j++) Log.print(values[j]+", ");
            ...
            if (toDo > 0) { // sample remaining part, if necessary
                ...
                /*
                 * Log.print("Chunk #"+chunks+" = ["); for (int j=0; j<toDo-1;
                 * j++) Log.print(values[j]+", ");
                 * Log.print(String.valueOf(values[toDo-1])); Log.println("]");
                 */
    ...
    /**
     * Tests different values for negaalphainv. Result: J.S. Vitter's
