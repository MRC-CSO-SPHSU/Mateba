...
public class Poisson extends AbstractDiscreteDistribution {
    ...
    @Serial
    private static final long serialVersionUID = 8979076893698561177L;
    ...
    protected static final double SWITCH_MEAN = 10.0; // switch from method A
    ...
    private static double f(int k, double l_nu, double c_pm) {
        return Math.exp(k * l_nu - DoubleArithmetic.logFactorial(k) - c_pm);
    }
            ...
            for (; ; ) {
                ...
                if ((U = gen.raw() * p6) < p2) { // centre left
                    ...
                    if ((V = f4 + f4 - W) < 1.0) { // quick reject of Y
                        Y = k4 - Dk;
                        if (V <= f4 + Dk * (1.0 - f4) / dr) { // quick accept
                            // of
                            return (Y); // Y = k4 - Dk
                        }
                        if (V <= f(Y, l_my, c_pm))
                ...
                if (Math.log(W) <= X * l_my - DoubleArithmetic.logFactorial(X) - c_pm)
                    return (X);
            }
