...
public class EmpiricalWalker extends AbstractDiscreteDistribution {
    @Serial
    private static final long serialVersionUID = 5609697653908300052L;
    ...
    public void setState2(double[] pdf) {
        ...
        // if (size < 1) {
        // throw new IllegalArgumentException("must have size greater than
        // zero");
        // }
        /*
        ...
        for (k = 0; k < size; ++k) {
            // if (pdf[k] < 0) {
            // throw new IllegalArgumentException("Probabilities must be >= 0:
            // "+pdf[k]);
            // }
            pTotal += pdf[k];
        }
        ...
        while (Smalls.size() > 0) {
            ...
            this.F[s] = size * E[s];
            /*
             * #if DEBUG fprintf(stderr,"s=%2d, A=%2d,
             * F=%.4f\n",s,(g->A)[s],(g->F)[s]); #endif
             */
            d = mean - E[s];
            E[s] += d; /* now E[s] == mean */
