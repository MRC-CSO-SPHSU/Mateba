 ...
 */
class FloatQuantileCalc extends Object {
    /**
    ...
    protected static long[] known_N_compute_B_and_K_quick(long N, float epsilon) {
        ...
        for (int b = 2; b <= maxBuffers; b++) {
            ...
            while (h <= maxHeight && // skip heights until x<=0
                    (h - 2) * ((float) Math.round(binomial(b + h - 2, h - 1)))
        ...
        for (int b = 2; b <= maxBuffers; b++) {
            ...
            if (h > Integer.MIN_VALUE) {
                float value = (Math.round(binomial(b + h - 2, h - 1)));
                long tmpK = ceiling(N_float / value);
    ...
    protected static long[] known_N_compute_B_and_K_slow(long N, float epsilon, float delta, int quantiles,
        ...
        for (long b = 2; b < maxBuffers; b++)
            for (long h = 3; h < maxHeight; h++) {
                float binomial = binomial(b + h - 2, h - 1);
                long tmp = ceiling(N_float / binomial);
                if ((b * tmp < memory)
                ...
                if (delta > 0.0) {
                    float t = (h - 2) * binomial(b + h - 2, h - 1) - binomial(b + h - 3, h - 3)
                            + binomial(b + h - 3, h - 2);
    ...
    public static void test_B_and_K_Calculation(String[] args) {
        ...
        float[] deltas = { 0.0f, 0.001f, 0.0001f, 0.00001f };

        float[] epsilons = { 0.0f, 0.1f, 0.05f, 0.01f, 0.005f, 0.001f, 0.0000001f };
        ...
        while (best_b == Long.MAX_VALUE && max_Iterations-- > 0) { // until we
            ...
            for (int b = 2; b <= max_b; b++) {
                for (int h = 2; h <= max_h; h++) {
                    float Ld = binomial(b + h - 2, h - 1);
                    float Ls = binomial(b + h - 3, h - 1);
