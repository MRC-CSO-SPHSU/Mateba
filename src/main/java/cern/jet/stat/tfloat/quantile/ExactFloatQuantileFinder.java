...
class ExactFloatQuantileFinder extends cern.colt.PersistentObject implements FloatQuantileFinder {
    ...
    public ExactFloatQuantileFinder() {
        this.buffer = new FloatArrayList(0);
        this.clear();
    }
    ...
    public float phi(float element) {
        this.sort();
        return cern.jet.stat.tfloat.FloatDescriptive.rankInterpolated(buffer, element) / this.size();
    }
    ...
    public FloatArrayList quantileElements(FloatArrayList phis) {
        this.sort();
        return cern.jet.stat.tfloat.FloatDescriptive.quantiles(this.buffer, phis);
        /*
         * int bufferSize = (int) this.size(); float[] quantileElements = new
         * float[phis.size()]; for (int i=phis.size(); --i >=0;) { int
         * rank=(int)Utils.epsilonCeiling(phis.get(i)*bufferSize) -1;
         * quantileElements[i]=buffer.get(rank); } return new
         * FloatArrayList(quantileElements);
         */
    }
