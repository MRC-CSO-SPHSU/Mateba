 ...
 */
public class DoubleDescriptive extends Object {
    /**
    ...
    public static void incrementalUpdate(DoubleArrayList data, int from, int to, double[] inOut) {
        ...
        for (; from <= to; from++) {
            ...
            /*
             * double oldDeviation = element - mean; mean += oldDeviation /
             * (N+1); sumSquaredDeviations += (element-mean)*oldDeviation; //
             * cool, huh?
             */

            /*
             * double oldMean = mean; mean += (element - mean)/(N+1); if (N > 0) {
    ...
    public static double moment(int k, double c, int size, double[] sumOfPowers) {
        ...
        for (int i = 0; i <= k; i++) {
            ...
            if (i == 0)
                ...
                y = Math.pow(c, i);
            // sum += sign *
            sum += sign * cern.jet.math.tdouble.DoubleArithmetic.binomial(k, i) * y * sumOfPowers[k - i];
            sign = -sign;
        }
        /*
