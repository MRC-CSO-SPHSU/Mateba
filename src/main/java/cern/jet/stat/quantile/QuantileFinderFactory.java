...
public class QuantileFinderFactory {
    ...
    protected static long[] known_N_compute_B_and_K_quick(long N, double epsilon) {
        ...
        for (int b = 2; b <= maxBuffers; b++) {
            ...
            if (h > Integer.MIN_VALUE) {
                double value = (DoubleArithmetic.binomial(b + h - 2, h - 1));
    ...
    public static QuantileFinder newQuantileFinder(boolean known_N, long N, double epsilon, double delta,
        ...
        if (epsilon <= 0.0 || N < 1000)
            return new ExactQuantileFinder();
        if (epsilon > 1)
            epsilon = 1;
        if (delta < 0)
            delta = 0;
        ...
        if (known_N) {
            ...
            if (b == 1)
                return new ExactQuantileFinder();
            return new KnownQuantileEstimator((int) b, (int) k, N, samplingRate[0], generator);
        } else {
            ...
            if (true) { // no maximum N provided by user.
                if (b1 == 1)
                    return new ExactQuantileFinder();
                return new UnknownQuantileEstimator((int) b1, (int) k1, (int) h1, preComputeEpsilon, generator);
            }
            ...
            if (b2 * k2 < b1 * k1) { // the KnownFinder is smaller
                if (b2 == 1)
                    return new ExactQuantileFinder();
                return new KnownQuantileEstimator((int) b2, (int) k2, N, samplingRate[0], generator);
            }
        ...
        while (best_b == Long.MAX_VALUE && max_Iterations-- > 0) { // until we
            ...
            for (int b = 2; b <= max_b; b++) {
                for (int h = 2; h <= max_h; h++) {
                    double Ld = DoubleArithmetic.binomial(b + h - 2, h - 1);
                    double Ls = DoubleArithmetic.binomial(b + h - 3, h - 1);
