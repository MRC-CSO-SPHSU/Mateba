 ...
 * <p>
 * This class stores a list <tt>l</tt> of <tt>double</tt> values for which
 * holds:
 ...
 */
public class EquiDepthHistogram implements Serializable, Cloneable {

    @Serial
    private static final long serialVersionUID = -9001719935864787224L;
     ...
     */
    public EquiDepthHistogram(double[] quantileElements) {
    ...
    public double phi(double element) {
        ...
        int index = java.util.Arrays.binarySearch(binBoundaries, element);
        // int index = new DoubleArrayList(binBoundaries).binarySearch(element);
        if (index >= 0) { // found
    ...
    @Override
    public EquiDepthHistogram clone() {
        try {
            EquiDepthHistogram clone = (EquiDepthHistogram) super.clone();
            clone.binBoundaries = this.binBoundaries.clone();
            return clone;
