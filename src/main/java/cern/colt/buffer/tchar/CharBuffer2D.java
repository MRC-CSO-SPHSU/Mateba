...
/**
 * Fixed sized (non-resizable) streaming buffer connected to a target {@link CharBuffer2DConsumer} to which data is
 * automatically flushed upon buffer overflow.
 */
public class CharBuffer2D implements CharBuffer2DConsumer, Serializable, Cloneable {

    @Serial
    private static final long serialVersionUID = -4923669456217947586L;
     ...
     *                 the target.
     */
    public CharBuffer2D(final @NotNull CharBuffer2DConsumer target, final int capacity) {
        this.target = target;
     ...
     */
    public void add(final char x, final char y) {
     ...
     */
    public void addAllOf(final @NotNull CharArrayList x, final @NotNull CharArrayList y) {
        val listSize = x.size();
    ...
    /**
     * Adds all internally buffered points to the receiver's target, then resetsthe current buffer size to zero.
     */
    public void flush() {
        if (this.size > 0) {
     ...
     */
    @Override
    public @NotNull CharBuffer2D clone() {
        try {
            val clone = (CharBuffer2D) super.clone();
