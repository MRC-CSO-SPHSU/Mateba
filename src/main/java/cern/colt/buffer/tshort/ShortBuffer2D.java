...
/**
 * Fixed sized (non-resizable) streaming buffer connected to a target {@link ShortBuffer2DConsumer} to which data is
 * automatically flushed upon buffer overflow.
 */
public class ShortBuffer2D implements ShortBuffer2DConsumer, Serializable, Cloneable {

    @Serial
    private static final long serialVersionUID = -191612574144407093L;
     ...
     *                 the target.
     */
    public ShortBuffer2D(final @NotNull ShortBuffer2DConsumer target, final int capacity) {
        this.target = target;
     ...
     */
    public void addAllOf(final @NotNull ShortArrayList x, final @NotNull ShortArrayList y) {
        val listSize = x.size();
     ...
     */
    @Override
    public @NotNull ShortBuffer2D clone() {
        try {
            val clone = (ShortBuffer2D) super.clone();
