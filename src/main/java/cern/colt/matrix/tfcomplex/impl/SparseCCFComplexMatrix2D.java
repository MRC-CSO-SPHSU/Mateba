...
public class SparseCCFComplexMatrix2D extends WrapperFComplexMatrix2D {
     ...
     *             .
     */
    public SparseCCFComplexMatrix2D(float[][] values) {
     ...
     */
    public SparseCCFComplexMatrix2D(int rows, int columns) {
     ...
     *             if <tt>rows<0 || columns<0</tt> .
     */
    public SparseCCFComplexMatrix2D(int rows, int columns, int nzmax) {
        super(null);
        try {
            setUp(rows, columns);
     ...
     */
    public SparseCCFComplexMatrix2D(int rows, int columns, int[] rowIndexes, int[] columnIndexes, float re, float im,
     ...
     */
    public SparseCCFComplexMatrix2D(int rows, int columns, int[] rowIndexes, int[] columnIndexes, float[] values,
    ...
    public FComplexMatrix2D assign(final cern.colt.function.tfcomplex.FComplexFComplexFunction function) {
        if (function instanceof cern.jet.math.tfcomplex.FComplexMult) { // x[i] = mult*x[i]
            ...
            final float[] valuesE = values;
            int nz = cardinality();
            float[] valE = new float[2];
            for (int j = 0; j < nz; j++) {
                valE[0] = valuesE[2 * j];
                valE[1] = valuesE[2 * j + 1];
                valE = FComplex.mult(valE, alpha);
                valuesE[2 * j] = valE[0];
    ...
    public FComplexMatrix2D assign(FComplexMatrix2D source) {
        ...
        if (source instanceof SparseCCFComplexMatrix2D) {
            SparseCCFComplexMatrix2D other = (SparseCCFComplexMatrix2D) source;
        ...
        } else if (source instanceof SparseRCFComplexMatrix2D) {
            SparseRCFComplexMatrix2D other = ((SparseRCFComplexMatrix2D) source).getConjugateTranspose();
    ...
    public FComplexMatrix2D assign(final FComplexMatrix2D y,
        ...
        if ((y instanceof SparseCCFComplexMatrix2D) && (function == cern.jet.math.tfcomplex.FComplexFunctions.plus)) { // x[i] = x[i] + y[i] 
            SparseCCFComplexMatrix2D yy = (SparseCCFComplexMatrix2D) y;
            int p, j, nz = 0, anz;
            ...
            x = new float[2 * m]; /* get workspace */
            SparseCCFComplexMatrix2D C = new SparseCCFComplexMatrix2D(m, n, anz + bnz); /* allocate result*/
            Cp = C.columnPointers;
            Ci = C.rowIndexes;
            Cx = C.values;
        ...
        if (function == cern.jet.math.tfcomplex.FComplexFunctions.mult) { // x[i] = x[i] * y[i]
            ...
            for (int j = columns; --j >= 0;) {
                ...
                for (int k = columnPointersA[j + 1]; --k >= low;) {
                    ...
                    valA = FComplex.mult(valA, y.getQuick(i, j));
                    valuesA[2 * k] = valA[0];
        ...
        if (function == cern.jet.math.tfcomplex.FComplexFunctions.div) { // x[i] = x[i] / y[i]
            ...
            for (int j = columns; --j >= 0;) {
                ...
                for (int k = columnPointersA[j + 1]; --k >= low;) {
                    ...
                    valA[0] = valuesA[2 * k];
                    valA[1] = valuesA[2 * k + 1];
                    valA = FComplex.div(valA, y.getQuick(i, j));
                    valuesA[2 * k] = valA[0];
                    valuesA[2 * k + 1] = valA[1];
                    if (valuesA[2 * k] == 0 && valuesA[2 * k + 1] == 0)
                        remove(i, j);
    ...
    public SparseRCFComplexMatrix2D getRowCompressed() {
        SparseCCFComplexMatrix2D tr = getConjugateTranspose();
        SparseRCFComplexMatrix2D rc = new SparseRCFComplexMatrix2D(rows, columns);
    ...
    public SparseCCFComplexMatrix2D getConjugateTranspose() {
        ...
        Ai = rowIndexes;
        Ax = values;
        SparseCCFComplexMatrix2D C = new SparseCCFComplexMatrix2D(columns, rows, Ai.length); /* allocate result */
        w = new int[m]; /* get workspace */
        Cp = C.columnPointers;
        Ci = C.rowIndexes;
        Cx = C.values;
    ...
    public FComplexMatrix2D like(int rows, int columns) {
        return new SparseCCFComplexMatrix2D(rows, columns);
    }
                            ...
                            if (threadID == 0) {
                                for (int i = firstColumn; i < lastColumn; i++) {
                                    ...
                                    for (int k = columnPointersA[i]; k < high; k++) {
                                        ...
                                        valA[0] = valuesA[2 * k];
                                        valA[1] = valuesA[2 * k + 1];
                                        valA = FComplex.mult(valA, yElem);
                                        valA = FComplex.mult(valA, alpha);
                ...
                for (int j = 0; j < nthreads; j++) {
                    ...
                    futures[j] = ConcurrencyUtils.submit(new Runnable() {
                        public void run() {
                            ...
                            float[] valA = new float[2];
                            float[] valY = new float[2];
                            float[] valZ = new float[2];
                            for (int i = firstColumn; i < lastColumn; i++) {
                                ...
                                for (int k = columnPointers[i]; k < high; k++) {
                                    ...
                                    sum = FComplex.plus(sum, FComplex.mult(valA, valY));
                                }
                                sum = FComplex.mult(alpha, sum);
                                valZ[0] = elementsZ[zidx];
                                valZ[1] = elementsZ[zidx + 1];
                                valZ = FComplex.mult(valZ, beta);
    ...
    public FComplexMatrix2D zMult(FComplexMatrix2D B, FComplexMatrix2D C, final float[] alpha, float[] beta,
        ...
        if (C == null) {
            if (B instanceof SparseCCFComplexMatrix2D) {
                C = new SparseCCFComplexMatrix2D(rowsA, p, (rowsA * p));
            } else {
        ...
        if ((B instanceof DenseFComplexMatrix2D) && (C instanceof DenseFComplexMatrix2D)) {
            ...
            if (!(alpha[0] == 1.0 && alpha[1] == 0)) {
                C.assign(cern.jet.math.tfcomplex.FComplexFunctions.mult(alpha));
            }

        } else if ((B instanceof SparseCCFComplexMatrix2D) && (C instanceof SparseCCFComplexMatrix2D)) {
            SparseCCFComplexMatrix2D AA;
            if (transposeA) {
                AA = getConjugateTranspose();
            } else {
            ...
            SparseCCFComplexMatrix2D BB = (SparseCCFComplexMatrix2D) B;
            if (transposeB) {
                BB = BB.getConjugateTranspose();
            }
            SparseCCFComplexMatrix2D CC = (SparseCCFComplexMatrix2D) C;
            ...
            for (j = 0; j < n; j++) {
                ...
                if (nz + m > nzmaxC) {
                    ...
                    Ci = rowIndexesNew;
                    float[] valuesNew = new float[2 * nzmaxC];
            ...
            Cp[n] = nz; /* finalize the last column of C */
            if (!(alpha[0] == 1.0 && alpha[1] == 0)) {
                CC.assign(cern.jet.math.tfcomplex.FComplexFunctions.mult(alpha));
            }
        } else {
            if (transposeB) {
    ...
    private int scatter(SparseCCFComplexMatrix2D A, int j, float[] beta, int[] w, float[] x, int mark,
            SparseCCFComplexMatrix2D C, int nz) {
        int i, p;
        int Ap[], Ai[], Ci[];
        float[] Ax;
        Ap = A.columnPointers;
