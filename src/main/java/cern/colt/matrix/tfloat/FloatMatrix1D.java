 ...
 * 
 */
public abstract class FloatMatrix1D extends AbstractMatrix1D {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Callable<Float>() {
                    ...
                    public Float call() throws Exception {
                        float a = f.apply(getQuick(indexElements[firstIdx]));
                        float elem;
                        for (int i = firstIdx + 1; i < lastIdx; i++) {
     ...
     * and is at least a <code>FloatMatrix1D</code> object that has the same
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Callable<float[]>() {
                    public float[] call() throws Exception {
                        ...
                        float maxValue = getQuick(location);
                        float elem;
                        for (int i = firstIdx + 1; i < lastIdx; i++) {
                            elem = getQuick(i);
                            if (maxValue < elem) {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Callable<float[]>() {
                    public float[] call() throws Exception {
                        int location = firstIdx;
                        float minValue = getQuick(location);
                        float elem;
                        for (int i = firstIdx + 1; i < lastIdx; i++) {
                            elem = getQuick(i);
                            if (minValue > elem) {
    ...
    public FloatMatrix1D viewFlip() {
        return (FloatMatrix1D) (view().vFlip());
    }
    ...
    public FloatMatrix1D viewPart(int index, int width) {
        return (FloatMatrix1D) (view().vPart(index, width));
    }
     ...
     * 	    new FloatProcedure() {
     * 	       public final boolean apply(float a) { return a % 2 == 0; }
     * 	    }
     * 	 );
     * 	 --&gt;
     * 	 matrix ==  0 2
     ...
     * @return the new view.
     */
    public FloatMatrix1D viewSelection(cern.colt.function.tfloat.FloatProcedure condition) {
     ...
     * information, see
     * {@link cern.colt.matrix.tfloat.algo.FloatSorting#sort(FloatMatrix1D)}.
    ...
    public FloatMatrix1D viewSorted() {
        return cern.colt.matrix.tfloat.algo.FloatSorting.mergeSort.sort(this);
    }
    ...
    public FloatMatrix1D viewStrides(int stride) {
        return (FloatMatrix1D) (view().vStrides(stride));
    }
     ...
     * @return a new view.
     */
    protected abstract FloatMatrix1D viewSelectionLike(int[] offsets);
