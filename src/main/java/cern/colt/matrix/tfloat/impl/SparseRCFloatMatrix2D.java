...
public class SparseRCFloatMatrix2D extends WrapperFloatMatrix2D {
     ...
     *             .
     */
    public SparseRCFloatMatrix2D(float[][] values) {
     ...
     */
    public SparseRCFloatMatrix2D(int rows, int columns, int[] rowIndexes, int[] columnIndexes, float value,
     ...
     *            if true, then column indexes are sorted
     */
    public SparseRCFloatMatrix2D(int rows, int columns, int[] rowIndexes, int[] columnIndexes, float[] values,
     ...
     *            numerical values
     */
    public SparseRCFloatMatrix2D(int rows, int columns, int[] rowPointers, int[] columnIndexes, float[] values) {
        super(null);
        try {
            setUp(rows, columns);
        } catch (IllegalArgumentException exc) { // we can hold rows*columns>Integer.MAX_VALUE cells !
    ...
    public FloatMatrix2D assign(FloatMatrix2D source) {
        ...
        if (source instanceof SparseRCFloatMatrix2D) {
            SparseRCFloatMatrix2D other = (SparseRCFloatMatrix2D) source;
        ...
        } else if (source instanceof SparseCCFloatMatrix2D) {
            SparseCCFloatMatrix2D other = ((SparseCCFloatMatrix2D) source).getTranspose();
    ...
    public FloatMatrix2D assign(final FloatMatrix2D y, cern.colt.function.tfloat.FloatFloatFunction function) {
        ...
        if ((y instanceof SparseRCFloatMatrix2D) && (function == cern.jet.math.tfloat.FloatFunctions.plus)) { // x[i] = x[i] + y[i] 
            SparseRCFloatMatrix2D yy = (SparseRCFloatMatrix2D) y;
            ...
            final float[] valuesC = new float[cnz];
            int nrow = rows;
            int ncol = columns;
    ...
    public SparseRCFloatMatrix2D getTranspose() {
        ...
        SparseRCFloatMatrix2D T = new SparseRCFloatMatrix2D(columns, rows);
    ...
    public void sortColumnIndexes() {
        SparseRCFloatMatrix2D T = getTranspose();
        this.rows = T.rows;
                        ...
                        public void run() {
                            if (threadID == 0) {
                                for (int i = firstRow; i < lastRow; i++) {
                                    int high = rowPointers[i + 1];
                                    float yElem = alpha * elementsY[zeroY + strideY * i];
    ...
    public FloatMatrix2D zMult(FloatMatrix2D B, FloatMatrix2D C, final float alpha, float beta,
        ...
        if (C == null) {
            if (B instanceof SparseRCFloatMatrix2D) {
                C = new SparseRCFloatMatrix2D(rowsA, p, (rowsA * p));
            } else {
        ...
        if ((B instanceof DenseFloatMatrix2D) && (C instanceof DenseFloatMatrix2D)) {
            ...
            for (int ii = 0; ii < rowsA; ii++) {
                ...
                for (int ka = rowPointersA[ii]; ka < highA; ka++) {
                    ...
                    CC.viewRow(ii).assign(BB.viewRow(jj), FloatFunctions.plusMultSecond(scal));
                }
            }
        } else if ((B instanceof SparseRCFloatMatrix2D) && (C instanceof SparseRCFloatMatrix2D)) {
            SparseRCFloatMatrix2D AA;
            SparseRCFloatMatrix2D BB;
            SparseRCFloatMatrix2D CC = (SparseRCFloatMatrix2D) C;
            if (transposeA) {
                AA = getTranspose();
            } else {
                AA = this;
            ...
            if (transposeB) {
                BB = ((SparseRCFloatMatrix2D) B).getTranspose();
            } else {
                BB = (SparseRCFloatMatrix2D) B;
            }
            ...
            for (int ii = 0; ii < rowsA; ii++) {
                ...
                //                FloatMatrix1D valuesCPart = valuesC.viewPart(rowPointersC[ii], length).viewSelection(indexes);
    ...
    private void realloc(int nzmax) {
        ...
        float[] valuesNew = new float[nzmax];
