...
public abstract class FloatMatrix2D extends AbstractMatrix2D {
        ...
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Callable<Float>() {
                    ...
                    public Float call() throws Exception {
                        float elem = getQuick(firstRow, 0);
                        float a = 0;
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Callable<Float>() {
                    ...
                    public Float call() throws Exception {
                        float a = f.apply(getQuick(rowElements[firstIdx], columnElements[firstIdx]));
                        float elem;
    ...
    public FloatMatrix2D viewSorted(int column) {
        return cern.colt.matrix.tfloat.algo.FloatSorting.mergeSort.sort(this, column);
    }
