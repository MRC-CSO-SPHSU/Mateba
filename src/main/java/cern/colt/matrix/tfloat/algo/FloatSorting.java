...
public class FloatSorting extends cern.colt.PersistentObject {
    ...
    /**
     * Compare two values, one of which is assumed to be Float.NaN
     */
     ...
     * FloatMatrix2D matrix = new DenseFloatMatrix2D(10000, 1000);
     ...
     * float[] aggregates = new float[matrix.rows()];
     * for (int i = matrix.rows(); --i &gt;= 0;)
     *     aggregates[i] = matrix.viewRow(i).aggregate(F.plus, F.log);
     * FloatMatrix2D sorted = quickSort(matrix, aggregates);
     * 
     * // THE SLOW VERSION (takes some 90 secs)
     * FloatMatrix1DComparator comparator = new FloatMatrix1DComparator() {
     *     public int compare(FloatMatrix1D x, FloatMatrix1D y) {
     *         float a = x.aggregate(F.plus, F.log);
     *         float b = y.aggregate(F.plus, F.log);
     *         return a &lt; b ? -1 : a == b ? 0 : 1;
     *     }
     * };
     * FloatMatrix2D sorted = quickSort(matrix, comparator);
     * </pre>
     * 
     * </td>
     * </table>
     ...
     */
    public FloatMatrix2D sort(final FloatMatrix2D matrix, final FloatMatrix1DComparator c) {
        int[] rowIndexes = new int[matrix.rows()]; // row indexes to reorder
        // instead of matrix itself
        for (int i = rowIndexes.length; --i >= 0;)
            rowIndexes[i] = i;

        final FloatMatrix1D[] views = new FloatMatrix1D[matrix.rows()]; // precompute
        // views
        // for
        // speed
     ...
     * FloatMatrix2D sorted = quickSort(matrix, hep.aida.bin.BinFunctions1D.median);
     * //FloatMatrix2D sorted = quickSort(matrix,hep.aida.bin.BinFunctions1D.sumOfLogarithms);
     * 
     * // THE SLOW VERSION (takes some 300 secs)
     * FloatMatrix1DComparator comparator = new FloatMatrix1DComparator() {
     *     public int compare(FloatMatrix1D x, FloatMatrix1D y) {
     *         float a = cern.colt.matrix.floatalgo.Statistic.bin(x).median();
     *         float b = cern.colt.matrix.floatalgo.Statistic.bin(y).median();
     *         // float a = x.aggregate(F.plus,F.log);
     *         // float b = y.aggregate(F.plus,F.log);
     *         return a &lt; b ? -1 : a == b ? 0 : 1;
     *     }
     ...
     *         matrix is left unaffected.</b>
     */
    public FloatMatrix2D sort(FloatMatrix2D matrix, hep.aida.tfloat.bin.FloatBinFunction1D aggregate) {
        ...
        hep.aida.tfloat.bin.FloatBinFunction1D[] func = { aggregate };
     ...
     *         the original matrix is left unaffected.</b>
     */
    public FloatMatrix3D sort(final FloatMatrix3D matrix, final FloatMatrix2DComparator c) {
        int[] sliceIndexes = new int[matrix.slices()]; // indexes to reorder
        // instead of matrix
        // itself
        for (int i = sliceIndexes.length; --i >= 0;)
        ...
        final FloatMatrix2D[] views = new FloatMatrix2D[matrix.slices()]; // precompute
        // views
        // for
        // speed
        for (int i = views.length; --i >= 0;)
    ...
    public static void zdemo3() {
        FloatSorting sort = quickSort;
        float[] values = { 0.5f, 1.5f, 2.5f, 3.5f };
    ...
    protected static void zdemo4() {
        float[] values1 = { 0, 1, 2, 3 };
        float[] values2 = { 0, 2, 4, 6 };
        FloatMatrix1D matrix1 = new DenseFloatMatrix1D(values1);
        FloatMatrix1D matrix2 = new DenseFloatMatrix1D(values2);
        System.out.println("m1:" + matrix1);
        System.out.println("m2:" + matrix2);

        matrix1.assign(matrix2, cern.jet.math.tfloat.FloatFunctions.pow);

        /*
         * matrix1.assign(matrix2, new cern.colt.function.FloatFloatFunction() {
         * public float apply(float x, float y) { return Math.pow(x,y); } } );
         */
    ...
    public static void zdemo5(int rows, int columns, boolean print) {
        ...
        timer.reset().start();
        // THE QUICK VERSION (takes some 10 secs)
        A = sort.sort(A, hep.aida.tfloat.bin.FloatBinFunctions1D.median);
        ...
        A = B;
        cern.colt.matrix.tfloat.algo.FloatMatrix1DComparator fun = new cern.colt.matrix.tfloat.algo.FloatMatrix1DComparator() {
            public int compare(FloatMatrix1D x, FloatMatrix1D y) {
                float a = cern.colt.matrix.tfloat.algo.FloatStatistic.bin(x).median();
                float b = cern.colt.matrix.tfloat.algo.FloatStatistic.bin(y).median();
                // float a = x.aggregate(F.plus,F.log);
                // float b = y.aggregate(F.plus,F.log);
                return a < b ? -1 : (a == b) ? 0 : 1;
            }
    ...
    public static void zdemo8(int size) {
        ...
        final cern.jet.math.tfloat.FloatFunctions F = cern.jet.math.tfloat.FloatFunctions.functions;
        FloatMatrix1D A = cern.colt.matrix.tfloat.FloatFactory1D.dense.random(size);
        System.out.print("now quick sorting... ");
        cern.colt.Timer timer = new cern.colt.Timer().start();
        quickSort.sort(A);
        timer.stop().display();
