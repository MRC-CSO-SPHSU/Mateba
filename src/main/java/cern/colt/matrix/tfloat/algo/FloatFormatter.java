...
public class FloatFormatter extends AbstractFormatter {
    ...
    public static void demo3(int size, float value) {
        ...
        String s;
        StringBuffer buf;
        FloatMatrix2D matrix = cern.colt.matrix.tfloat.FloatFactory2D.dense.make(size, size, value);
    ...
    public String toTitleString(FloatMatrix2D matrix, String[] rowNames, String[] columnNames, String rowAxisName,
        ...
        FloatMatrix2D rowStats = matrix.like(matrix.rows(), aggr.length); // hold
        // row
        // aggregations
        FloatMatrix2D colStats = matrix.like(aggr.length, matrix.columns()); // hold
        // column
        // aggregations

        cern.colt.matrix.tfloat.algo.FloatStatistic.aggregate(matrix, aggr, colStats); // aggregate
        // an
        // entire
        // column
        // at a
        // time
        cern.colt.matrix.tfloat.algo.FloatStatistic.aggregate(matrix.viewDice(), aggr, rowStats.viewDice()); // aggregate
        // an
        // entire
        // row
        // at a
        ...
        FloatMatrix2D tmp = matrix.like(matrix.rows() + aggr.length, matrix.columns());
