...
public class DenseFloatLUDecompositionQuick implements java.io.Serializable {
    ...
    transient protected float[] workFloat;
    ...
    public void decompose(FloatMatrix2D A) {
        ...
        cern.colt.list.tint.IntArrayList nonZeroIndexes = new cern.colt.list.tint.IntArrayList(); // sparsity
        FloatMatrix1D LUcolj = LU.viewColumn(0).like(); // blocked column j
        cern.jet.math.tfloat.FloatMult multFunction = cern.jet.math.tfloat.FloatMult.mult(0);
        ...
        for (int j = 0; j < n; j++) {
            ...
            // sparsity detection
            int maxCardinality = m / CUT_OFF; // == heuristic depending on
            // speedup
            ...
            for (int i = 0; i < m; i++) {
                ...
                float before = LUcolj.getQuick(i);
            ...
            if (p < m) {
                ...
                for (int i = j + 1; i < m; i++) {
                    float v = Math.abs(LUcolj.getQuick(i));
                    if (v > max) {
                        p = i;
                        max = v;
                    }
    ...
    /**
     * Returns pivot permutation vector as a one-dimensional float array
     * 
     * @return (float) piv
     */
    ...
    protected boolean isNonsingular(FloatMatrix2D matrix) {
        ...
        float epsilon = algebra.property().tolerance(); // consider
        // numerical
        // instability
    ...
    public void solve(FloatMatrix1D B) {
        ...
        algebra.permute(B, this.piv, this.workFloat);
        ...
        // Solve L*Y = B(piv,:)
        for (int k = 0; k < n; k++) {
            float f = B.getQuick(k);
            if (f != 0) {
                for (int i = k + 1; i < n; i++) {
                    // B[i] -= B[k]*LU[i][k];
                    float v = LU.getQuick(i, k);
                    if (v != 0)
    ...
    public void solve(FloatMatrix2D B) {
        ...
        // if (this.work2 == null || this.work2.length < m) this.work2 = new
        // int[m];
        ...
        // transformations
        cern.jet.math.tfloat.FloatMult div = cern.jet.math.tfloat.FloatMult.div(0);
        cern.jet.math.tfloat.FloatPlusMultSecond minusMult = cern.jet.math.tfloat.FloatPlusMultSecond.minusMult(0);

        cern.colt.list.tint.IntArrayList nonZeroIndexes = new cern.colt.list.tint.IntArrayList(); // sparsity
        FloatMatrix1D Browk = cern.colt.matrix.tfloat.FloatFactory1D.dense.make(nx); // blocked
        // row
        // k
        ...
        for (int k = 0; k < n; k++) {
            ...
            // sparsity detection
            int maxCardinality = nx / CUT_OFF; // == heuristic depending on
            // speedup
            ...
            for (int i = k + 1; i < n; i++) {
                ...
                // for (int j = 0; j < nx; j++) B.set(i,j, B.get(i,j) -
                // B.get(k,j)*LU.get(i,k));
        ...
        for (int k = n - 1; k >= 0; k--) {
            ...
            // for (int j = 0; j < nx; j++) B.set(k,j, B.get(k,j) /
            // LU.get(k,k));
            ...
            // blocking
            if (Browk == null)
                Browk = cern.colt.matrix.tfloat.FloatFactory1D.dense.make(B.columns());
            Browk.assign(Brows[k]);
    ...
    public String toString() {
        ...
        buf.append("\n\ninverse(A) = ");
        FloatMatrix2D identity = cern.colt.matrix.tfloat.FloatFactory2D.dense.identity(LU.rows());
        try {
            this.solve(identity);
