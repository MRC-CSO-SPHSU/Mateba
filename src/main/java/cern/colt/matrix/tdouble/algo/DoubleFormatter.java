...
public class DoubleFormatter extends AbstractFormatter {
    ...
    public static void demo1() {
        ...
        DoubleMatrix2D matrix = cern.colt.matrix.tdouble.DoubleFactory2D.dense.make(values);
    ...
    public static void demo3(int size, double value) {
        ...
        String s;
        StringBuffer buf;
        DoubleMatrix2D matrix = cern.colt.matrix.tdouble.DoubleFactory2D.dense.make(size, size, value);
        ...
        timer.reset().start();
        cern.colt.matrix.Former format = new cern.colt.matrix.FormerFactory().create("%G");
        buf = new StringBuffer();
        for (int i = size; --i >= 0;) {
            for (int j = size; --j >= 0;) {
    ...
    public static void demo5() {
        ...
        System.out.println(cern.colt.matrix.tdouble.DoubleFactory2D.dense.make(values));
        System.out.println(new DoubleFormatter("%G").toTitleString(cern.colt.matrix.tdouble.DoubleFactory2D.dense
    ...
    public static void demo6() {
        ...
        // Formatter().toSourceCode(cern.colt.matrix.DoubleFactory2D.dense.make(values)));
        System.out.println(new DoubleFormatter().toString(cern.colt.matrix.tdouble.DoubleFactory2D.dense.make(values)));
        System.out.println(new DoubleFormatter().toTitleString(cern.colt.matrix.tdouble.DoubleFactory2D.dense
                .make(values), rowNames, columnNames, "vendor", "density", "title"));
    }
    ...
    public String toTitleString(DoubleMatrix2D matrix, String[] rowNames, String[] columnNames, String rowAxisName,
        ...
        DoubleMatrix2D rowStats = matrix.like(matrix.rows(), aggr.length); // hold
        // row
        // aggregations
        DoubleMatrix2D colStats = matrix.like(aggr.length, matrix.columns()); // hold
        // column
        // aggregations
        ...
        if (columnNames != null) {
            cern.colt.list.tobject.ObjectArrayList list = new cern.colt.list.tobject.ObjectArrayList(columnNames);
            list.add("|");
            for (int i = 0; i < aggr.length; i++)
                list.add(aggr[i].name()); // add names of aggregation
            // functions
        ...
        if (rowNames != null) {
            cern.colt.list.tobject.ObjectArrayList list = new cern.colt.list.tobject.ObjectArrayList(rowNames);
            for (int i = 0; i < aggr.length; i++)
                list.add(aggr[i].name()); // add names of aggregation
            // functions
