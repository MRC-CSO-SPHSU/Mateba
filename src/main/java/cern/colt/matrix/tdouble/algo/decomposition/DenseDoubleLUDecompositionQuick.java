...
public class DenseDoubleLUDecompositionQuick implements java.io.Serializable {
    ...
    public void solve(DoubleMatrix2D B) {
        ...
        cern.colt.list.tint.IntArrayList nonZeroIndexes = new cern.colt.list.tint.IntArrayList(); // sparsity
        DoubleMatrix1D Browk = cern.colt.matrix.tdouble.DoubleFactory1D.dense.make(nx); // blocked
        // row
        // k
        ...
        for (int k = n - 1; k >= 0; k--) {
            ...
            // blocking
            if (Browk == null)
                Browk = cern.colt.matrix.tdouble.DoubleFactory1D.dense.make(B.columns());
            Browk.assign(Brows[k]);
    ...
    public String toString() {
        ...
        buf.append("\n\ninverse(A) = ");
        DoubleMatrix2D identity = cern.colt.matrix.tdouble.DoubleFactory2D.dense.identity(LU.rows());
        try {
            this.solve(identity);
