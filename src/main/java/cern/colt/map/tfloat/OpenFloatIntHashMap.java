 ...
 */
public class OpenFloatIntHashMap extends AbstractFloatIntMap {
    /**
     * 
     */
     ...
     */
    public OpenFloatIntHashMap(int initialCapacity) {
    ...
    public Object clone() {
        OpenFloatIntHashMap copy = (OpenFloatIntHashMap) super.clone();
    ...
    protected int indexOfInsertion(float key) {
        final float tab[] = table;
        final byte stat[] = state;
        final int length = tab.length;

        final int hash = HashFunctions.hash(key) & 0x7FFFFFFF;
        int i = hash % length;
        int decrement = hash % (length - 2); // float hashing, see
    ...
    public boolean put(float key, int value) {
    ...
    protected void rehash(int newCapacity) {
        ...
        float newTable[] = new float[newCapacity];
