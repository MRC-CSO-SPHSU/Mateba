...
public final class Version {
    ...
    private static int[] numbers() {
         ...
         * getPackage().getImplementationVersion(); if (s==null) return numbers;
         * int k = s.indexOf('('); s = s.substring(0,k); s = s.trim(); //s =
         * s.replace('.', ' '); //s = ViolinStrings.Strings.stripBlanks(s); //s =
         * ViolinStrings.Strings.translate(s, ".", " "); String[] words =
         * s.split("."); // requires jdk 1.4.x for (int i=0; i<w; i++) {
         * numbers[i] = Integer.parseInt(words[i]); //numbers[i] =
         * Integer.parseInt(ViolinStrings.Strings.word(s, i));
         * //System.out.println(numbers[i]); } return numbers;
         */
    }
}
