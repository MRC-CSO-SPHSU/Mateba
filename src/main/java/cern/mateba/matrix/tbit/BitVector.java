 ...
 */
package cern.mateba.matrix.tbit;
...
public class BitVector implements Serializable, Cloneable {
    @Serial
    private static final long serialVersionUID = -1927105235001745357L;
    /*
     ...
     * performance is critical. Constructs a bit vector with the given backing
     * bits and size. <b>WARNING:</b> For efficiency reasons and to keep memory
     * usage low, <b>the array is not copied</b>. So if subsequently you modify
    ...
    public boolean equals(Object obj) {
        if (obj == null || !(obj instanceof BitVector other))
            return false;
        if (this == obj)
            return true;
     ...
     * @param procedure a procedure object taking as argument the current bit index.
    ...
    public boolean forEachIndexFromToInState(int from, int to, boolean state,
                                             cern.mateba.function.tint.IntProcedure procedure) {
        /*
         ...
         * checkRangeFromTo(from, to, nbits); final long[] theBits = this.bits; //
         * cached for speed int length=to-from+1; for (int i=from; --length >=
         * 0; i++) { if (QuickBitVector.get(theBits,i)==state) { if
         * (!function.apply(i)) return false; } } return true;
         */

        /*
         * This low level implementation exploits the fact that for any full
         * unit one can determine in O(1) whether it contains at least one true
         * bit, and whether it contains at least one false bit. Thus, 64 bits
         * can often be skipped with one simple comparison if the vector is
         * either sparse or dense.
         *
         * However, careful coding must be done for leading and/or trailing
         * units which are not entirely contained in the query range.
         */
        ...
        // If there is a trailing partial unit, then there is one full unit less
        // to test.
        ...
        // (It does not matter that iterating over partial units is a little bit
        // slow,
        // the only thing that matters is that iterating over full units is
        // quick.)
        long comparator;
        if (state)
            comparator = 0L;
    ...
    public void replaceFromToWith(int from, int to, BitVector source, int sourceFrom) {
        ...
        if (source.bits == this.bits && from <= sourceFrom && sourceFrom <= to) { // dangerous
            // intersection
            source = source.copy();
        }
        ...
        /*
         * This version is equivalent to the version below but 20 times
         * slower... for (int i=from; --length >= 0; i++, sourceFrom++) {
         * QuickBitVector.put(theBits,i,QuickBitVector.get(sourceBits,sourceFrom)); }
         */

        // Low level implementation for speed.
        // This could be done even faster by implementing on even lower levels.
        // But then the code would probably become a "don't touch" piece.
    ...
    private class IndexProcedure implements cern.mateba.function.tint.IntProcedure {
