...
public class BitMatrix implements Serializable, Cloneable {
    ...
    /*
     * The bits of this matrix. bits are stored in row major, i.e.
     ...
     */
    public boolean forEachCoordinateInState(boolean state, final cern.mateba.function.tint.IntIntProcedure procedure) {
        /*
         * this is equivalent to the low level version below, apart from that it
         * iterates in the reverse oder and is slower. if (size()==0) return
         * true; BitVector vector = toBitVector(); return
         * vector.forEachIndexFromToInState(0,size()-1,state, new
         * cern.mateba.function.IntFunction() { public boolean apply(int index) {
