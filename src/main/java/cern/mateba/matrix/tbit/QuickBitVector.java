...
public class QuickBitVector {
    ...
    public static long getLongFromTo(long[] bits, int from, int to) {
        ...
        final int fromIndex = from >> ADDRESS_BITS_PER_UNIT; // equivalent to
        // from/64
        ...
        if (fromIndex == toIndex) { // range does not cross unit boundaries;
            // value to retrieve is contained in one
            // single long value.
    ...
    private static long[] precomputePows() {
        ...
        // OLD STUFF
        /*
         * for (int i=BITS_PER_UNIT+1; --i >= 0; ) { pows[i]=value; value =
         * value >>> 1; System.out.println((i)+":"+pows[i]); }
         */

        /*
         * long[] pows=new long[BITS_PER_UNIT]; for (int i=0; i<BITS_PER_UNIT-1;
         * i++) { pows[i]=Math.round(Math.pow(2.0,i+1))-1;
         * System.out.println((i)+":"+pows[i]); } pows[BITS_PER_UNIT-1] = ~0L;
    ...
    public static void putLongFromTo(long[] bits, long value, int from, int to) {
        ...
        final int toOffset = to & BIT_INDEX_MASK;
        /*
         * this is equivalent to the above, but slower: int
         * fromIndex=from/BITS_PER_UNIT; int toIndex=to/BITS_PER_UNIT; int
         * fromOffset=from%BITS_PER_UNIT; int toOffset=to%BITS_PER_UNIT;
         */
