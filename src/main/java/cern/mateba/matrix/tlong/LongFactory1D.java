...
import cern.mateba.matrix.tlong.impl.SparseLongMatrix1D;
...
public class LongFactory1D implements Serializable, Cloneable {
     ...
     * @return a new list.
     */
    public cern.mateba.list.tlong.LongArrayList toList(LongMatrix1D values) {
        int size = (int) values.size();
        cern.mateba.list.tlong.LongArrayList list = new cern.mateba.list.tlong.LongArrayList(size);
        list.setSize(size);
        for (int i = size; --i >= 0;)
