...
public abstract class LongMatrix2D extends AbstractMatrix2D {
    ...
    @Serial
    private static final long serialVersionUID = 3755859006088807240L;
     ...
     * <tt>a(1) == f(get(0,0)), a(0)==Long.NaN</tt>.
     * <p>
     * <b>Example:</b>
     *
     * <pre>
        ...
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(getQuick(firstRow, 0));
                    int d = 1;
    ...
    public long aggregate(final cern.mateba.function.tlong.LongLongFunction aggr,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(getQuick(rowElements[firstIdx], columnElements[firstIdx]));
                    long elem;
     ...
     * and terminators are
     * <tt>a(1) == f(get(0,0),other.get(0,0)), a(0)==Long.NaN</tt>.
     * <p>
     * <b>Example:</b>
     *
     * <pre>
    ...
    public long aggregate(final LongMatrix2D other, final cern.mateba.function.tlong.LongLongFunction aggr,
        ...
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastRow = (j == nthreads - 1) ? rows : firstRow + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(getQuick(firstRow, 0), other.getQuick(firstRow, 0));
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
     ...
     * @see cern.jet.math.tlong.LongFunctions
     */
    public LongMatrix2D assign(final cern.mateba.function.tlong.LongFunction f) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
            Future<?>[] futures = new Future[nthreads];
     ...
     * and is at least a <code>LongMatrix2D</code> object that has the same
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tlong.algo.LongProperty.DEFAULT.equals(this, (LongMatrix2D) obj);
    }
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    long maxValue1 = getQuick(rowLocation1, 0);
                    int d = 1;
                    long elem;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int rowLocation1 = firstRow;
                    int columnLocation1 = 0;
                    long minValue1 = getQuick(rowLocation1, 0);
                    int d = 1;
                    long elem;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
    ...
    public String toString() {
        return new cern.mateba.matrix.tlong.algo.LongFormatter().toString(this);
    }
     ...
     * {@link cern.mateba.matrix.tlong.algo.LongSorting#sort(LongMatrix2D, int)}.
    ...
    public LongMatrix2D viewSorted(int column) {
        return cern.mateba.matrix.tlong.algo.LongSorting.mergeSort.sort(this, column);
    }
