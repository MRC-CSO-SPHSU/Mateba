...
public abstract class LongMatrix1D extends AbstractMatrix1D {
    ...
    @Serial
    private static final long serialVersionUID = -3004206440746384819L;
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(getQuick(firstIdx));
     ...
     */
    public long aggregate(final cern.mateba.function.tlong.LongLongFunction aggr,
                          final cern.mateba.function.tlong.LongFunction f, final IntArrayList indexList) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastIdx = (j == nthreads - 1) ? size : firstIdx + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(getQuick(indexElements[firstIdx]));
                    long elem;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
    ...
    public long aggregate(final LongMatrix1D other, final cern.mateba.function.tlong.LongLongFunction aggr,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastIdx = (j == nthreads - 1) ? size : firstIdx + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(getQuick(firstIdx), other.getQuick(firstIdx));
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
     ...
     * @see cern.jet.math.tlong.LongFunctions
     */
    public LongMatrix1D assign(final cern.mateba.function.tlong.LongFunction f) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
            Future<?>[] futures = new Future[nthreads];
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tlong.algo.LongProperty.DEFAULT.equals(this, (LongMatrix1D) obj);
    }
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    long maxValue1 = getQuick(location1);
                    long elem;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        elem = getQuick(i);
                        if (maxValue1 < elem) {
                    ...
                    return new long[]{maxValue1, location1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public long[] getMinLocation() {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int location1 = firstIdx;
                    long minValue1 = getQuick(location1);
                    long elem;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        elem = getQuick(i);
                        if (minValue1 > elem) {
                    ...
                    return new long[]{minValue1, location1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public LongMatrix1D viewSorted() {
        return cern.mateba.matrix.tlong.algo.LongSorting.mergeSort.sort(this);
    }
