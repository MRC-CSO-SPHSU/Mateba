...
public class SparseRCLongMatrix2D extends WrapperLongMatrix2D {
    @Serial
    private static final long serialVersionUID = 2486030274803588391L;
    ...
    public LongMatrix2D assign(final cern.mateba.function.tlong.LongFunction function) {
        if (function instanceof cern.jet.math.tlong.LongMult) { // x[i] = mult*x[i]
            ...
            forEachNonZero(new cern.mateba.function.tlong.IntIntLongFunction() {
    ...
    public LongMatrix2D assign(LongMatrix2D source) {
        ...
        if (source instanceof SparseRCLongMatrix2D other) {
            ...
            assign(0);
            source.forEachNonZero(new cern.mateba.function.tlong.IntIntLongFunction() {
                public long apply(int i, int j, long value) {
                    setQuick(i, j, value);
                    return value;
                }
    ...
    public LongMatrix2D assign(final LongMatrix2D y, cern.mateba.function.tlong.LongLongFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCLongMatrix2D yy) && (function == cern.jet.math.tlong.LongFunctions.plus)) { // x[i] = x[i] + y[i]
        ...
        if (function instanceof cern.jet.math.tlong.LongPlusMultSecond) { // x[i] = x[i] + alpha*y[i]
            ...
            if (alpha == 0)
                return this; // nothing to do
            y.forEachNonZero(new cern.mateba.function.tlong.IntIntLongFunction() {
                public long apply(int i, int j, long value) {
    ...
    public LongMatrix2D zMult(LongMatrix2D B, LongMatrix2D C, final long alpha, long beta, final boolean transposeA,
        ...
        } else if ((B instanceof SparseRCLongMatrix2D) && (C instanceof SparseRCLongMatrix2D CC)) {
            SparseRCLongMatrix2D AA;
