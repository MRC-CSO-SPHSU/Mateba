...
public class DenseLongMatrix3D extends LongMatrix3D {
    ...
    @Serial
    private static final long serialVersionUID = 7245505990771475834L;
    /**
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(elements[zero + firstSlice * sliceStride]);
                    int d = 1;
    ...
    public long aggregate(final cern.mateba.function.tlong.LongLongFunction aggr,
                          final cern.mateba.function.tlong.LongFunction f, final IntArrayList sliceList, final IntArrayList rowList,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(elements[zero + sliceElements[firstIdx] * sliceStride + rowElements[firstIdx]
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    long a1 = f.apply(elements[idx], elemsOther[idxOther]);
                    int d = 1;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                            for (int c = d; c < columns; c++) {
    ...
    public LongMatrix3D assign(final cern.mateba.function.tlong.LongFunction function) {
        final int zero = (int) index(0, 0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            nthreads = Math.min(nthreads, slices);
    ...
    public LongMatrix3D assign(final cern.mateba.function.tlong.LongProcedure cond, final long value) {
        final int zero = (int) index(0, 0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            nthreads = Math.min(nthreads, slices);
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int row_loc1 = 0;
                    int col_loc1 = 0;
                    long maxValue1 = elements[zero + firstSlice * sliceStride];
                    int d = 1;
                    long elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new long[]{maxValue1, slice_loc1, row_loc1, col_loc1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int row_loc1 = 0;
                    int col_loc1 = 0;
                    long minValue1 = elements[zero + slice_loc1 * sliceStride];
                    int d = 1;
                    long elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new long[]{minValue1, slice_loc1, row_loc1, col_loc1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
