...
import cern.mateba.map.tlong.AbstractLongLongMap;
import cern.mateba.map.tlong.OpenLongLongHashMap;
 ...
 * {@link cern.mateba.map.tlong.OpenLongLongHashMap}, which is a compact and
 ...
 * @see cern.mateba.map
 * @see cern.mateba.map.tlong.OpenLongLongHashMap
 */
public class SparseLongMatrix2D extends LongMatrix2D {
     ...
     */
    @Serial
    private static final long serialVersionUID = -6327592258610052218L;
     ...
     * to memory usage see {@link cern.mateba.map.tlong.OpenLongLongHashMap}.
     *
     * @param rows            the number of rows the matrix shall have.
     * @param columns         the number of columns the matrix shall have.
    ...
    public LongMatrix2D assign(cern.mateba.function.tlong.LongFunction function) {
    ...
    public LongMatrix2D assign(LongMatrix2D source) {
        // overriden for performance only
        if (!(source instanceof SparseLongMatrix2D other)) {
    ...
    public LongMatrix2D assign(final LongMatrix2D y, cern.mateba.function.tlong.LongLongFunction function) {
        ...
        if (function instanceof cern.jet.math.tlong.LongPlusMultSecond) { // x[i] = x[i] + alpha*y[i]
            ...
            this.elements.forEachPair(new cern.mateba.function.tlong.LongLongProcedure() {
    ...
    public SparseLongMatrix2D assign(final int[] rowIndexes, final int[] columnIndexes, final long value,
                                     final cern.mateba.function.tlong.LongLongFunction function) {
