...
public class DenseLongMatrix2D extends LongMatrix2D {
    ...
    @Serial
    private static final long serialVersionUID = 6874377248087095225L;
    /**
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(elements[zero + firstRow * rowStride]);
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
                            a1 = aggr.apply(a1, f.apply(elements[zero + r * rowStride + c * columnStride]));
                        }
                        d = 0;
                    }
                    return a1;
    ...
    public long aggregate(final cern.mateba.function.tlong.LongLongFunction aggr,
                          final cern.mateba.function.tlong.LongFunction f, final cern.mateba.function.tlong.LongProcedure cond) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        final int zero = (int) index(0, 0);
        long a = 0;
    ...
    public long aggregate(final cern.mateba.function.tlong.LongLongFunction aggr,
                          final cern.mateba.function.tlong.LongFunction f, final IntArrayList rowList, final IntArrayList columnList) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        final int zero = (int) index(0, 0);
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(elements[zero + rowElements[firstIdx] * rowStride + columnElements[firstIdx]
                        * columnStride]);
                    long elem;
    ...
    public long aggregate(final LongMatrix2D other, final cern.mateba.function.tlong.LongLongFunction aggr,
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastRow = (j == nthreads - 1) ? rows : firstRow + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long a1 = f.apply(elements[zero + firstRow * rowStride], elemsOther[zeroOther + firstRow
                        * rowStrideOther]);
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
    ...
    public LongMatrix2D assign(final cern.mateba.function.tlong.LongProcedure cond,
    ...
    public LongMatrix2D assign(final cern.mateba.function.tlong.LongProcedure cond, final long value) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
    ...
    public LongMatrix2D assign(final LongMatrix2D source) {
        ...
        if (!(source instanceof DenseLongMatrix2D other)) {
            super.assign(source);
            return this;
        }
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int i = idx, j1 = idxOther, c = 0; c < columns; c++) {
                            elements[i] = elemsOther[j1];
                            i += columnStride;
                            j1 += columnStrideOther;
                        }
                        idx += rowStride;
    ...
    public LongMatrix2D assign(final LongMatrix2D y, final cern.mateba.function.tlong.LongLongFunction function) {
        // overriden for performance only
        if (!(y instanceof DenseLongMatrix2D other)) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    if (function == cern.jet.math.tlong.LongFunctions.mult) {
                        ...
                        if (multiplicator == 1) {
                            ...
                            for (int r = firstRow; r < lastRow; r++) {
                                for (int i = idx, j1 = idxOther, c = 0; c < columns; c++) {
                                    elements[i] += multiplicator * elemsOther[j1];
                                    i += columnStride;
                                    j1 += columnStrideOther;
                                }
                                idx += rowStride;
                        ...
                        for (int r = firstRow; r < lastRow; r++) {
                            for (int i = idx, j1 = idxOther, c = 0; c < columns; c++) {
                                elements[i] = function.apply(elements[i], elemsOther[j1]);
                                i += columnStride;
                                j1 += columnStrideOther;
                            }
                            idx += rowStride;
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    return Long.valueOf(cardinality1);
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public long[] getMaxLocation() {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    long maxValue1 = elements[zero + firstRow * rowStride];
                    int rowLocation1 = firstRow;
                    int colLocation = 0;
                    long elem;
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                    ...
                    return new long[]{maxValue1, rowLocation1, colLocation};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    long minValue1 = elements[zero + firstRow * rowStride];
                    long elem;
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
    ...
    private LongMatrix2D zMultSequential(LongMatrix2D B, LongMatrix2D C, long alpha, long beta, boolean transposeA,
        ...
        if (!(B instanceof DenseLongMatrix2D BB) || !(C instanceof DenseLongMatrix2D CC))
            return super.zMult(B, C, alpha, beta, transposeA, transposeB);
        if (B.rows() != columnsA)
    ...
    protected boolean haveSharedCellsRaw(LongMatrix2D other) {
        if (other instanceof SelectedDenseLongMatrix2D otherMatrix) {
            return this.elements == otherMatrix.elements;
        } else if (other instanceof DenseLongMatrix2D otherMatrix) {
            return this.elements == otherMatrix.elements;
        }
        return false;
    }
