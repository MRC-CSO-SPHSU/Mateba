...
public abstract class IntMatrix3D extends AbstractMatrix3D {
    @Serial
    private static final long serialVersionUID = 1368746815436254936L;
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(firstSlice, 0, 0));
                    int d = 1;
     ...
     */
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr,
                         final cern.mateba.function.tint.IntFunction f, final IntArrayList sliceList, final IntArrayList rowList,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(sliceElements[firstIdx], rowElements[firstIdx],
    ...
    public int aggregate(final IntMatrix3D other, final cern.mateba.function.tint.IntIntFunction aggr,
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastSlice = (j == nthreads - 1) ? slices : firstSlice + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(firstSlice, 0, 0), other.getQuick(firstSlice, 0, 0));
                    int d = 1;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                            for (int c = d; c < columns; c++) {
     ...
     * @see cern.jet.math.tint.IntFunctions
     */
    public IntMatrix3D assign(final cern.mateba.function.tint.IntFunction function) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            nthreads = Math.min(nthreads, slices);
            Future<?>[] futures = new Future[nthreads];
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tint.algo.IntProperty.DEFAULT.equals(this, (IntMatrix3D) obj);
    }
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int maxValue1 = getQuick(sliceLocation1, 0, 0);
                    int d = 1;
                    int elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new int[]{maxValue1, sliceLocation1, rowLocation1, columnLocation1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int rowLocation1 = 0;
                    int columnLocation1 = 0;
                    int minValue1 = getQuick(sliceLocation1, 0, 0);
                    int d = 1;
                    int elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new int[]{minValue1, sliceLocation1, rowLocation1, columnLocation1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public IntMatrix3D viewColumnFlip() {
        return (IntMatrix3D) (view().vColumnFlip());
    }
    ...
    public IntMatrix3D viewRowFlip() {
        return (IntMatrix3D) (view().vRowFlip());
    }
    ...
    public IntMatrix3D viewSliceFlip() {
        return (IntMatrix3D) (view().vSliceFlip());
    }
