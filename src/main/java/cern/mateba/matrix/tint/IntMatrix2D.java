...
public abstract class IntMatrix2D extends AbstractMatrix2D {
    ...
    @Serial
    private static final long serialVersionUID = -6344428428821967825L;
        ...
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(firstRow, 0));
                    int d = 1;
    ...
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr,
                         final cern.mateba.function.tint.IntFunction f, final cern.mateba.function.tint.IntProcedure cond) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        int a = 0;
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
     ...
     */
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr,
                         final cern.mateba.function.tint.IntFunction f, final IntArrayList rowList, final IntArrayList columnList) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(rowElements[firstIdx], columnElements[firstIdx]));
                    int elem;
    ...
    public int aggregate(final IntMatrix2D other, final cern.mateba.function.tint.IntIntFunction aggr,
        ...
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastRow = (j == nthreads - 1) ? rows : firstRow + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(firstRow, 0), other.getQuick(firstRow, 0));
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
     ...
     * @see cern.jet.math.tint.IntFunctions
     */
    public IntMatrix2D assign(final cern.mateba.function.tint.IntFunction f) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
            Future<?>[] futures = new Future[nthreads];
     ...
     * @return <tt>this</tt> (for convenience only).
     */
    public IntMatrix2D assign(final cern.mateba.function.tint.IntProcedure cond, final int value) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
            Future<?>[] futures = new Future[nthreads];
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tint.algo.IntProperty.DEFAULT.equals(this, (IntMatrix2D) obj);
    }
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int maxValue1 = getQuick(rowLocation1, 0);
                    int d = 1;
                    int elem;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
                    ...
                    return new int[]{maxValue1, rowLocation1, columnLocation1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int rowLocation1 = firstRow;
                    int columnLocation1 = 0;
                    int minValue1 = getQuick(rowLocation1, 0);
                    int d = 1;
                    int elem;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
                    ...
                    return new int[]{minValue1, rowLocation1, columnLocation1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
     ...
     * {@link cern.mateba.matrix.tint.algo.IntSorting#sort(IntMatrix2D, int)}. For
    ...
    public IntMatrix2D viewSorted(int column) {
        return cern.mateba.matrix.tint.algo.IntSorting.mergeSort.sort(this, column);
    }
