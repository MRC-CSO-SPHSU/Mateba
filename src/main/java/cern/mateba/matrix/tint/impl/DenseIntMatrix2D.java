...
public class DenseIntMatrix2D extends IntMatrix2D {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + firstRow * rowStride]);
                    int d = 1;
    ...
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + rowElements[firstIdx] * rowStride + columnElements[firstIdx]
                        * columnStride]);
                    int elem;
    ...
    public int aggregate(final IntMatrix2D other, final cern.mateba.function.tint.IntIntFunction aggr,
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastRow = (j == nthreads - 1) ? rows : firstRow + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + firstRow * rowStride], elemsOther[zeroOther + firstRow
                        * rowStrideOther]);
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
    ...
    public IntMatrix2D assign(final cern.mateba.function.tint.IntFunction function) {
    ...
    public int[] getMaxLocation() {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int maxValue1 = elements[zero + firstRow * rowStride];
                    ...
                    return new int[]{maxValue1, rowLocation1, colLocation};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int minValue1 = elements[zero + firstRow * rowStride];
                    int elem;
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
