...
public class DenseColumnIntMatrix2D extends IntMatrix2D {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + (rows - 1) * rowStride + (firstColumn - 1) * columnStride]);
                    int d = 1;
    ...
    public int aggregate(final IntIntFunction aggr, final IntFunction f, final IntArrayList rowList,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + rowElements[firstIdx - 1] * rowStride
    ...
    public IntMatrix2D assign(final IntMatrix2D source) {
        ...
        if (!(source instanceof DenseColumnIntMatrix2D other)) {
            super.assign(source);
            return this;
        }
        if (other == this)
    ...
    public IntMatrix2D assign(final IntMatrix2D y, final IntIntFunction function) {
        ...
        if (!(y instanceof DenseColumnIntMatrix2D other)) {
    ...
    public int[] getMaxLocation() {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastColumn = (j == (nthreads - 1)) ? 0 : firstColumn - k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int maxValue1 = elements[zero + (rows - 1) * rowStride + (firstColumn - 1) * columnStride];
    ...
    public int[] getMinLocation() {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastColumn = (j == (nthreads - 1)) ? 0 : firstColumn - k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int minValue1 = elements[zero + (rows - 1) * rowStride + (firstColumn - 1) * columnStride];
                    int rowLocation1 = rows - 1;
                    int columnLocation1 = firstColumn - 1;
                    int elem;
                    int d = 1;
