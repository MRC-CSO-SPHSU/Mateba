...
public class SparseRCIntMatrix2D extends WrapperIntMatrix2D {
    ...
    public IntMatrix2D assign(final cern.mateba.function.tint.IntFunction function) {
        if (function instanceof cern.jet.math.tint.IntMult) { // x[i] = mult*x[i]
            ...
            forEachNonZero(new cern.mateba.function.tint.IntIntIntFunction() {
    ...
    public IntMatrix2D assign(IntMatrix2D source) {
        ...
        if (source instanceof SparseRCIntMatrix2D other) {
            ...
            assign(0);
            source.forEachNonZero(new cern.mateba.function.tint.IntIntIntFunction() {
                public int apply(int i, int j, int value) {
                    setQuick(i, j, value);
                    return value;
                }
    ...
    public IntMatrix2D assign(final IntMatrix2D y, cern.mateba.function.tint.IntIntFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCIntMatrix2D yy) && (function == cern.jet.math.tint.IntFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            int cnz = Math.max(columnIndexes.length, Math.min(Integer.MAX_VALUE, rowPointers[rows]
                + rowPointersY[rows]));
        ...
        if (function instanceof cern.jet.math.tint.IntPlusMultSecond) { // x[i] = x[i] + alpha*y[i]
            ...
            if (alpha == 0)
                return this; // nothing to do
            y.forEachNonZero(new cern.mateba.function.tint.IntIntIntFunction() {
                public int apply(int i, int j, int value) {
                        ...
                        if (threadID == 0) {
                            for (int i = firstRow; i < lastRow; i++) {
                                ...
                                for (int k1 = rowPointers[i]; k1 < high; k1++) {
                                    ...
                                    elementsZ[zeroZ + strideZ * j1] += values[k1] * yElem;
                                }
                            }
                        } else {
                            for (int i = firstRow; i < lastRow; i++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    if (beta == 0.0) {
                        for (int i = firstRow; i < lastRow; i++) {
                            ...
                            for (; k12 < high; k12++) {
                                sum += values[k12] * elementsY[columnIndexes[k12]];
                            }
    ...
    public IntMatrix2D zMult(IntMatrix2D B, IntMatrix2D C, final int alpha, int beta, final boolean transposeA,
        ...
        } else if ((B instanceof SparseRCIntMatrix2D) && (C instanceof SparseRCIntMatrix2D CC)) {
            SparseRCIntMatrix2D AA;
