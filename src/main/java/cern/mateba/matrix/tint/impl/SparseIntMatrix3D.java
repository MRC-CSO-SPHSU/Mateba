 ...
 * {@link cern.mateba.map.tlong.OpenLongIntHashMap}, which is a compact and
...
public class SparseIntMatrix3D extends IntMatrix3D {
    @Serial
    private static final long serialVersionUID = 254612537170269233L;
    /*
     ...
     * @throws IllegalArgumentException if <tt>(int)slices*columns*rows > Integer.MAX_VALUE</tt>.
     ...
     * {@link cern.mateba.map.tlong.OpenLongIntHashMap}.
     *
     * @param slices          the number of slices the matrix shall have.
     * @param rows            the number of rows the matrix shall have.
     * @param columns         the number of columns the matrix shall have.
    ...
    protected boolean haveSharedCellsRaw(IntMatrix3D other) {
        if (other instanceof SelectedSparseIntMatrix3D otherMatrix) {
            return this.elements == otherMatrix.elements;
        } else if (other instanceof SparseIntMatrix3D otherMatrix) {
            return this.elements == otherMatrix.elements;
        }
        return false;
    }
