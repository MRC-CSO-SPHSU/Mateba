...
public class DenseIntMatrix1D extends IntMatrix1D {
    ...
    @Serial
    private static final long serialVersionUID = 6159303282827919873L;
    /**
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int a1 = f.apply(elements[idx]);
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        idx += stride;
                        a1 = aggr.apply(a1, f.apply(elements[idx]));
                    }
                    return a1;
                });
            }
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int a1 = f.apply(elements[idx], elemsOther[idxOther]);
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        idx += stride;
    ...
    public IntMatrix1D assign(final cern.mateba.function.tint.IntFunction function) {
    ...
    public int[] getMaxLocation() {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int idx = zero + firstIdx * stride;
                    int maxValue1 = elements[idx];
                    int location1 = (idx - zero) / stride;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        idx += stride;
                        if (maxValue1 < elements[idx]) {
                            maxValue1 = elements[idx];
                            location1 = (idx - zero) / stride;
                        }
                    }
    ...
    public int[] getMinLocation() {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int idx = zero + firstIdx * stride;
                    int minValue1 = elements[idx];
                    int location1 = (idx - zero) / stride;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        idx += stride;
                        if (minValue1 > elements[idx]) {
                            minValue1 = elements[idx];
                            location1 = (idx - zero) / stride;
                        }
                    }
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    for (int k1 = min / 4; --k1 >= 0; ) {
                        sum1 += elements[idx += stride] * elemsOther[idxOther += strideOtherF]
        ...
        if ((nthreads > 1) && (length >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int min = lastIdx - firstIdx;
                    for (int k1 = min / 4; --k1 >= 0; ) {
                        sum1 += elements[idx += stride] * elementsOther[idxOther += strideOtherF]
