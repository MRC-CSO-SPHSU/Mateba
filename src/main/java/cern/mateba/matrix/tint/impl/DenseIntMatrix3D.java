...
public class DenseIntMatrix3D extends IntMatrix3D {
    ...
    @Serial
    private static final long serialVersionUID = -7685585476605663070L;
    /**
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + firstSlice * sliceStride]);
                    int d = 1;
    ...
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(elements[zero + sliceElements[firstIdx] * sliceStride + rowElements[firstIdx]
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int row_loc1 = 0;
                    int col_loc1 = 0;
                    int maxValue1 = elements[zero + firstSlice * sliceStride];
                    int d = 1;
                    int elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new int[]{maxValue1, slice_loc1, row_loc1, col_loc1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int row_loc1 = 0;
                    int col_loc1 = 0;
                    int minValue1 = elements[zero + slice_loc1 * sliceStride];
                    int d = 1;
                    int elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new int[]{minValue1, slice_loc1, row_loc1, col_loc1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
