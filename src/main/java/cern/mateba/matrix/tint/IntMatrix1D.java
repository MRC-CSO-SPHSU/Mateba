...
public abstract class IntMatrix1D extends AbstractMatrix1D {
     ...
     */
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr, final cern.mateba.function.tint.IntFunction f) {
        if (size == 0)
            throw new IllegalArgumentException("size == 0");
        int a = f.apply(getQuick(0));
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(firstIdx));
     ...
     * @see cern.jet.math.tint.IntFunctions
     */
    public int aggregate(final cern.mateba.function.tint.IntIntFunction aggr,
                         final cern.mateba.function.tint.IntFunction f, final IntArrayList indexList) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastIdx = (j == nthreads - 1) ? size : firstIdx + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(indexElements[firstIdx]));
                    int elem;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
    ...
    public int aggregate(final IntMatrix1D other, final cern.mateba.function.tint.IntIntFunction aggr,
        ...
        if (size == 0)
            throw new IllegalArgumentException("size == 0");
        int a = f.apply(getQuick(0), other.getQuick(0));
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
            Future<?>[] futures = new Future[nthreads];
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastIdx = (j == nthreads - 1) ? size : firstIdx + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int a1 = f.apply(getQuick(firstIdx), other.getQuick(firstIdx));
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
     ...
     * @see cern.jet.math.tint.IntFunctions
     */
    public IntMatrix1D assign(final cern.mateba.function.tint.IntFunction f) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
            Future<?>[] futures = new Future[nthreads];
     ...
     * @return <tt>this</tt> (for convenience only).
     */
    public IntMatrix1D assign(final cern.mateba.function.tint.IntProcedure cond, final int value) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
            Future<?>[] futures = new Future[nthreads];
     ...
     * @see cern.jet.math.tint.IntFunctions
     */
    public IntMatrix1D assign(final IntMatrix1D y, final cern.mateba.function.tint.IntIntFunction function) {
        checkSize(y);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
     ...
     * @see cern.jet.math.tint.IntFunctions
     */
    public IntMatrix1D assign(IntMatrix1D y, cern.mateba.function.tint.IntIntFunction function,
                              cern.mateba.list.tint.IntArrayList nonZeroIndexes) {
        checkSize(y);
        int[] nonZeroElements = nonZeroIndexes.elements();

        // specialized for speed
        if (function == cern.jet.math.tint.IntFunctions.mult) { // x[i] = x[i] *
            // y[i]
            int j = 0;
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tint.algo.IntProperty.DEFAULT.equals(this, (IntMatrix1D) obj);
    }
     ...
     * @return a new empty matrix of the same dynamic type.
     */
    public abstract IntMatrix1D like(int size);
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int maxValue1 = getQuick(location1);
                    int elem;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        elem = getQuick(i);
                        if (maxValue1 < elem) {
                    ...
                    return new int[]{maxValue1, location1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public int[] getMinLocation() {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int location1 = firstIdx;
                    int minValue1 = getQuick(location1);
                    int elem;
                    for (int i = firstIdx + 1; i < lastIdx; i++) {
                        elem = getQuick(i);
                        if (minValue1 > elem) {
                    ...
                    return new int[]{minValue1, location1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public String toString() {
        return new cern.mateba.matrix.tint.algo.IntFormatter().toString(this);
    }
    ...
    public IntMatrix1D viewPart(int index, int width) {
        return (IntMatrix1D) (view().vPart(index, width));
    }
     ...
     * @return the new view.
     */
    public IntMatrix1D viewSelection(cern.mateba.function.tint.IntProcedure condition) {
     ...
     * @return a new view.
     */
    protected abstract IntMatrix1D viewSelectionLike(int[] offsets);
     ...
     * information, see
     * {@link cern.mateba.matrix.tint.algo.IntSorting#sort(IntMatrix1D)}. For more
    ...
    public IntMatrix1D viewSorted() {
        return cern.mateba.matrix.tint.algo.IntSorting.mergeSort.sort(this);
    }
    ...
    public IntMatrix1D viewStrides(int stride) {
        return (IntMatrix1D) (view().vStrides(stride));
    }
