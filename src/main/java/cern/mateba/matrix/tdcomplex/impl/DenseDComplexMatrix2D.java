...
public class DenseDComplexMatrix2D extends DComplexMatrix2D {
    ...
    @Serial
    private static final long serialVersionUID = -7756254378374691841L;
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    double[] a1 = f.apply(elements[idx], elements[idx + 1]);
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
                            idx = zero + r * rowStride + c * columnStride;
                            a1 = aggr.apply(a1, f.apply(elements[idx], elements[idx + 1]));
                        }
                        d = 0;
                    }
                    return a1;
    ...
    public DComplexMatrix2D assign(final cern.mateba.function.tdcomplex.DComplexRealFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
    ...
    public DComplexMatrix2D assign(final DComplexMatrix2D source) {
        ...
        if (!(source instanceof DenseDComplexMatrix2D other)) {
            super.assign(source);
            return this;
        }
        if (other == this)
                    ...
                    if (function == cern.jet.math.tcomplex.DComplexFunctions.mult) {
                        for (int r = firstRow; r < lastRow; r++) {
                            for (int i = idx, j1 = idxOther, c = 0; c < columns; c++) {
                                tmp1[0] = elements[i];
                                tmp1[1] = elements[i + 1];
                                tmp2[0] = elemsOther[j1];
                                tmp2[1] = elemsOther[j1 + 1];
    ...
    protected DComplexMatrix2D zMultSeq(DComplexMatrix2D B, DComplexMatrix2D C, double[] alpha, double[] beta,
        ...
        if (!(C instanceof DenseDComplexMatrix2D CC))
