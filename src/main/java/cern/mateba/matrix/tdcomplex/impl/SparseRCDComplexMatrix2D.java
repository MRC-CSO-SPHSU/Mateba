...
public class SparseRCDComplexMatrix2D extends WrapperDComplexMatrix2D {
     ...
     * @param re               real part of numerical value
     * @param im               imaginary part of numerical value
    ...
    public DComplexMatrix2D assign(final cern.mateba.function.tdcomplex.DComplexDComplexFunction function) {
        if (function instanceof cern.jet.math.tcomplex.DComplexMult) { // x[i] = mult*x[i]
            ...
            forEachNonZero(new cern.mateba.function.tdcomplex.IntIntDComplexFunction() {
    ...
    public DComplexMatrix2D assign(DComplexMatrix2D source) {
        ...
        if (source instanceof SparseRCDComplexMatrix2D other) {
            ...
            assign(0, 0);
            source.forEachNonZero(new cern.mateba.function.tdcomplex.IntIntDComplexFunction() {
                public double[] apply(int i, int j, double[] value) {
                    setQuick(i, j, value);
                    return value;
                }
    ...
    public DComplexMatrix2D assign(final DComplexMatrix2D y, cern.mateba.function.tdcomplex.DComplexDComplexDComplexFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCDComplexMatrix2D yy) && (function == cern.jet.math.tcomplex.DComplexFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            int cnz = Math.max(columnIndexes.length, (int) Math.min(Integer.MAX_VALUE, (long) rowPointers[rows] + (long) rowPointersY[rows]));
            final int[] columnIndexesC = new int[cnz];
            final double[] valuesC = new double[2 * cnz];
            int nrow = rows;
            int ncol = columns;
            int nzmax = cnz;
            if (function == cern.jet.math.tcomplex.DComplexFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
    ...
    public synchronized double[] getQuick(int row, int column) {
        // int k = cern.mateba.Sorting.binarySearchFromTo(columnIndexes, column, rowPointers[row], rowPointers[row + 1] - 1);
    ...
    public String toString() {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = rowPointers[i]; j < high; j++) {
                if (values[2 * j + 1] > 0) {
                    builder.append('(').append(i).append(',').append(columnIndexes[j]).append(')').append('\t').append(values[2 * j]).append('+').append(values[2 * j + 1]).append('i').append('\n');
                } else if (values[2 * j + 1] == 0) {
                    builder.append('(').append(i).append(',').append(columnIndexes[j]).append(')').append('\t').append(values[2 * j]).append('\n');
                } else {
                    builder.append('(').append(i).append(',').append(columnIndexes[j]).append(')').append('\t').append(values[2 * j]).append('-').append(values[2 * j + 1]).append('i').append('\n');
                }
            }
    ...
    public DComplexMatrix1D zMult(DComplexMatrix1D y, DComplexMatrix1D z, final double[] alpha, final double[] beta, final boolean transposeA) {
    ...
    public DComplexMatrix2D zMult(DComplexMatrix2D B, DComplexMatrix2D C, final double[] alpha, double[] beta, final boolean transposeA, boolean transposeB) {
        int rowsA = rows;
        ...
        if ((B instanceof DenseDComplexMatrix2D) && (C instanceof DenseDComplexMatrix2D CC)) {
        ...
        } else if ((B instanceof SparseRCDComplexMatrix2D) && (C instanceof SparseRCDComplexMatrix2D CC)) {
            SparseRCDComplexMatrix2D AA;
            ...
            for (int ii = 0; ii < rowsA; ii++) {
                ...
                // int[] indexes = cern.mateba.matrix.tint.algo.IntSorting.quickSort.sortIndex(columnIndexesCPart);
            ...
            final cern.jet.math.tcomplex.DComplexPlusMultSecond fun = cern.jet.math.tcomplex.DComplexPlusMultSecond.plusMult(new double[2]);
