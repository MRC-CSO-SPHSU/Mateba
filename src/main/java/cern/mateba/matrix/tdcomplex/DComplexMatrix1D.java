...
public abstract class DComplexMatrix1D extends AbstractMatrix1D {
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tdcomplex.algo.DComplexProperty.DEFAULT.equals(this, (DComplexMatrix1D) obj);
    }
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    for (int k1 = firstIdx; k1 < lastIdx; k1++) {
                        ...
                        sum1 = DComplex.plus(sum1, DComplex.mult(tmp, getQuick(idx)));
                    }
                    return sum1;
                });
            }
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    double[] sum1 = new double[2];
                    for (int k1 = firstIdx; k1 < lastIdx; k1++) {
                        sum1 = DComplex.plus(sum1, getQuick(k1));
                    }
                    return sum1;
                });
            }
