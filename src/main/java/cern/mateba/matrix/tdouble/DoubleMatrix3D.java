...
/**
 * Abstract base class for 3-d matrices holding <tt>double</tt> elements. First
...
public abstract class DoubleMatrix3D extends AbstractMatrix3D {
    ...
    @Serial
    private static final long serialVersionUID = 2525762765349705501L;
     ...
     * are <tt>a(1) == f(get(0,0,0)), a(0)==Double.NaN</tt>.
     * <p>
     * <b>Example:</b>
     *
     * <pre>
     ...
     */
    public double aggregate(final cern.mateba.function.tdouble.DoubleDoubleFunction aggr,
                            final cern.mateba.function.tdouble.DoubleFunction f, final cern.mateba.function.tdouble.DoubleProcedure cond) {
        if (size() == 0)
            return Double.NaN;
        double a = 0;
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
     ...
     */
    public double aggregate(final cern.mateba.function.tdouble.DoubleDoubleFunction aggr,
                            final cern.mateba.function.tdouble.DoubleFunction f, final IntArrayList sliceList,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    double a1 = f.apply(getQuick(sliceElements[firstIdx], rowElements[firstIdx],
     ...
     * and terminators are
     * <tt>a(1) == f(get(0,0,0),other.get(0,0,0)), a(0)==Double.NaN</tt>.
     * <p>
     * <b>Example:</b>
     *
     * <pre>
     ...
     * @see cern.jet.math.tdouble.DoubleFunctions
     */
    public DoubleMatrix3D assign(final cern.mateba.function.tdouble.DoubleFunction function) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            nthreads = Math.min(nthreads, slices);
            Future<?>[] futures = new Future[nthreads];
     ...
     * @return <tt>this</tt> (for convenience only).
     */
    public DoubleMatrix3D assign(final cern.mateba.function.tdouble.DoubleProcedure cond, final double value) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            nthreads = Math.min(nthreads, slices);
            Future<?>[] futures = new Future[nthreads];
     ...
     * @see cern.jet.math.tdouble.DoubleFunctions
     */
    public DoubleMatrix3D assign(final DoubleMatrix3D y, final cern.mateba.function.tdouble.DoubleDoubleFunction function) {
        checkShape(y);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            nthreads = Math.min(nthreads, slices);
    ...
    public DoubleMatrix3D assign(final DoubleMatrix3D y,
                                 final cern.mateba.function.tdouble.DoubleDoubleFunction function, final IntArrayList sliceList,
                                 final IntArrayList rowList, final IntArrayList columnList) {
        checkShape(y);
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tdouble.algo.DoubleProperty.DEFAULT.equals(this, (DoubleMatrix3D) obj);
    }
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    double maxValue1 = getQuick(sliceLocation1, 0, 0);
                    int d = 1;
                    double elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new double[]{maxValue1, sliceLocation1, rowLocation1, columnLocation1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    int rowLocation1 = 0;
                    int columnLocation1 = 0;
                    double minValue1 = getQuick(sliceLocation1, 0, 0);
                    int d = 1;
                    double elem;
                    for (int s = firstSlice; s < lastSlice; s++) {
                        for (int r = 0; r < rows; r++) {
                    ...
                    return new double[]{minValue1, sliceLocation1, rowLocation1, columnLocation1};
                });
            }
            try {
                for (int j = 0; j < nthreads; j++) {
    ...
    public String toString() {
        return new cern.mateba.matrix.tdouble.algo.DoubleFormatter().toString(this);
    }
     ...
     * information, see
     * {@link cern.mateba.matrix.tdouble.algo.DoubleSorting#sort(DoubleMatrix3D, int, int)}
    ...
    public DoubleMatrix3D viewSorted(int row, int column) {
        return cern.mateba.matrix.tdouble.algo.DoubleSorting.mergeSort.sort(this, row, column);
    }
     ...
     *
     * cern.mateba.function.Double27Function f = new
     * cern.mateba.function.Double27Function() {    public final
