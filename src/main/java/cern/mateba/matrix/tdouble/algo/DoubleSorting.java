...
import cern.mateba.matrix.tdouble.DoubleFactory3D;
...
public class DoubleSorting implements Serializable, Cloneable{
    ...
    public static final DoubleSorting mergeSort = new DoubleSorting() {
        ...
        protected void runSort(int[] a, int fromIndex, int toIndex, IntComparator c) {
            cern.mateba.Sorting.mergeSort(a, fromIndex, toIndex, c);
        }

        protected void runSort(int fromIndex, int toIndex, IntComparator c, cern.mateba.Swapper swapper) {
            cern.mateba.GenericSorting.mergeSort(fromIndex, toIndex, c, swapper);
        }
    };
    @Serial
    ...
    protected void runSort(int[] a, int fromIndex, int toIndex, IntComparator c) {
        cern.mateba.Sorting.parallelQuickSort(a, fromIndex, toIndex, c);
    }

    protected void runSort(int fromIndex, int toIndex, IntComparator c, cern.mateba.Swapper swapper) {
        cern.mateba.GenericSorting.quickSort(fromIndex, toIndex, c, swapper);
    }
     ...
     * DoubleMatrix1DComparator comparator = new DoubleMatrix1DComparator() {
     *     public int compare(DoubleMatrix1D x, DoubleMatrix1D y) {
     *         double a = cern.mateba.matrix.doublealgo.Statistic.bin(x).median();
     *         double b = cern.mateba.matrix.doublealgo.Statistic.bin(y).median();
     *         // double a = x.aggregate(F.plus,F.log);
     *         // double b = y.aggregate(F.plus,F.log);
     *         return a &lt; b ? -1 : a == b ? 0 : 1;
     *     }
    ...
    public static void zdemo3() {
        ...
        /*
         * sorted.assign( new cern.mateba.function.DoubleFunction() { public
    ...
    protected static void zdemo4() {
        ...
        /*
         * matrix1.assign(matrix2, new cern.mateba.function.DoubleDoubleFunction() {
    ...
    public static void zdemo5(int rows, int columns, boolean print) {
        ...
        DoubleMatrix2D A = cern.mateba.matrix.tdouble.DoubleFactory2D.dense.make(rows, columns);
        ...
        A = B;
        cern.mateba.matrix.tdouble.algo.DoubleMatrix1DComparator fun = new cern.mateba.matrix.tdouble.algo.DoubleMatrix1DComparator() {
            public int compare(DoubleMatrix1D x, DoubleMatrix1D y) {
                double a = DoubleStatistic.bin(x).median();
                double b = cern.mateba.matrix.tdouble.algo.DoubleStatistic.bin(y).median();
                // double a = x.aggregate(F.plus,F.log);
                // double b = y.aggregate(F.plus,F.log);
                return a < b ? -1 : (a == b) ? 0 : 1;
            }
    ...
    public static void zdemo8(int size) {
        ...
        final cern.jet.math.tdouble.DoubleFunctions F = cern.jet.math.tdouble.DoubleFunctions.functions;
        DoubleMatrix1D A = cern.mateba.matrix.tdouble.DoubleFactory1D.dense.random(size);
        System.out.print("now quick sorting... ");
        cern.mateba.Timer timer = new cern.mateba.Timer().start();
        quickSort.sort(A);
        timer.stop().display();
