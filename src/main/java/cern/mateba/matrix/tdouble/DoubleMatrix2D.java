...
public abstract class DoubleMatrix2D extends AbstractMatrix2D {
    ...
    public double aggregate(final cern.mateba.function.tdouble.DoubleDoubleFunction aggr,
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    double a1 = f.apply(getQuick(rowElements[firstIdx], columnElements[firstIdx]));
                    double elem;
     ...
     * @see cern.jet.math.tdouble.DoubleFunctions
     */
    public DoubleMatrix2D assign(final cern.mateba.function.tdouble.DoubleFunction f) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
            Future<?>[] futures = new Future[nthreads];
    ...
    public boolean equals(Object obj) {
        ...
        return cern.mateba.matrix.tdouble.algo.DoubleProperty.DEFAULT.equals(this, (DoubleMatrix2D) obj);
    }
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    double maxValue1 = getQuick(rowLocation1, 0);
                    int d = 1;
                    double elem;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    int rowLocation1 = firstRow;
                    int columnLocation1 = 0;
                    double minValue1 = getQuick(rowLocation1, 0);
                    int d = 1;
                    double elem;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
