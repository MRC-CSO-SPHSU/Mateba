...
public class SparseCCDoubleMatrix2D extends WrapperDoubleMatrix2D {
     ...
     * @param columnPointers columns pointers
    ...
    public DoubleMatrix2D assign(final cern.mateba.function.tdouble.DoubleFunction function) {
        if (function instanceof cern.jet.math.tdouble.DoubleMult) { // x[i] = mult*x[i]
            ...
            forEachNonZero(new cern.mateba.function.tdouble.IntIntDoubleFunction() {
    ...
    public DoubleMatrix2D assign(DoubleMatrix2D source) {
        ...
        if (source instanceof SparseCCDoubleMatrix2D other) {
            ...
            assign(0);
            source.forEachNonZero(new cern.mateba.function.tdouble.IntIntDoubleFunction() {
                public double apply(int i, int j, double value) {
                    setQuick(i, j, value);
                    return value;
                }
    ...
    public DoubleMatrix2D assign(final DoubleMatrix2D y, cern.mateba.function.tdouble.DoubleDoubleFunction function) {
        ...
        if ((y instanceof SparseCCDoubleMatrix2D yy) && (function == cern.jet.math.tdouble.DoubleFunctions.plus)) { // x[i] = x[i] + y[i]
        ...
        if (function instanceof cern.jet.math.tdouble.DoublePlusMultSecond) { // x[i] = x[i] + alpha*y[i]
            ...
            if (alpha == 0)
                return this; // nothing to do
            y.forEachNonZero(new cern.mateba.function.tdouble.IntIntDoubleFunction() {
                public double apply(int i, int j, double value) {
    ...
    public synchronized double getQuick(int row, int column) {
        //        int k = cern.mateba.Sorting.binarySearchFromTo(dcs.i, row, dcs.p[column], dcs.p[column + 1] - 1);
                        ...
                        if (threadID == 0) {
                            for (int i = firstColumn; i < lastColumn; i++) {
                                ...
                                for (int k1 = columnPointersA[i]; k1 < high; k1++) {
                                    ...
                                    elementsZ[zeroZ + strideZ * j1] += valuesA[k1] * yElem;
                                }
                            }
                        } else {
                            for (int i = firstColumn; i < lastColumn; i++) {
                ...
                for (int j = 0; j < nthreads; j++) {
                    ...
                    futures[j] = ConcurrencyUtils.submit(() -> {
                        ...
                        for (int i = firstColumn; i < lastColumn; i++) {
                            ...
                            for (; k12 < high; k12++) {
                                sum += valuesA[k12] * elementsY[dcs.i[k12]];
                            }
    ...
    public DoubleMatrix2D zMult(DoubleMatrix2D B, DoubleMatrix2D C, final double alpha, double beta,
        ...
        } else if ((B instanceof SparseCCDoubleMatrix2D BB) && (C instanceof SparseCCDoubleMatrix2D CC)) {
            SparseCCDoubleMatrix2D AA;
            if (transposeA) {
                AA = getTranspose();
            } else {
