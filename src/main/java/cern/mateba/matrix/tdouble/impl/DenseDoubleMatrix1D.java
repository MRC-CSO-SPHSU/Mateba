...
public class DenseDoubleMatrix1D extends DoubleMatrix1D {
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    ...
                    double a1 = f.apply(elements[idx]);
                    for (int i = firstIdx - 1; --i >= lastIdx; ) {
                        a1 = aggr.apply(a1, f.apply(elements[idx -= stride]));
                    }
                    return a1;
                });
            }
    ...
    public DoubleMatrix1D assign(final cern.mateba.function.tdouble.DoubleFunction function) {
