...
public class DenseObjectMatrix2D extends ObjectMatrix2D {
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    Object a1 = f.apply(elements[zero + firstRow * rowStride]);
                    int d = 1;
    ...
    public Object aggregate(final cern.mateba.function.tobject.ObjectObjectFunction aggr,
                            final cern.mateba.function.tobject.ObjectFunction f, final IntArrayList rowList, final IntArrayList columnList) {
        if (size() == 0)
            throw new IllegalArgumentException("size == 0");
        final int zero = (int) index(0, 0);
        ...
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(() -> {
                    Object a1 = f.apply(elements[zero + rowElements[firstIdx] * rowStride + columnElements[firstIdx]
                        * columnStride]);
                    Object elem;
    ...
    public Object aggregate(final ObjectMatrix2D other, final cern.mateba.function.tobject.ObjectObjectFunction aggr,
        ...
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                final int lastRow = (j == nthreads - 1) ? rows : firstRow + k;
                futures[j] = ConcurrencyUtils.submit(() -> {
                    Object a1 = f.apply(elements[zero + firstRow * rowStride], elemsOther[zeroOther + firstRow
                        * rowStrideOther]);
                    int d = 1;
                    for (int r = firstRow; r < lastRow; r++) {
                        for (int c = d; c < columns; c++) {
    ...
    public ObjectMatrix2D assign(final cern.mateba.function.tobject.ObjectProcedure cond, final Object value) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
