package cern.mateba.matrix.tobject.algo;
 ...
 * @see cern.mateba.Partitioning "Partitioning arrays (provides more
 * documentation)"
 */
public class ObjectPartitioning {
    ...
    /**
     * Same as
     * {@link cern.mateba.Partitioning#partition(int[], int, int, int[], int, int, int[])}
     ...
     * @param rowIndexes   the index of the i-th row; is modified by this method to
     ...
     * @param splitIndexes a list into which this method fills the indexes of rows
    ...
    public static void partition(ObjectMatrix2D matrix, int[] rowIndexes, int rowFrom, int rowTo, int column,
        ...
        cern.mateba.Partitioning.genericPartition(rowFrom, rowTo, splitFrom, splitTo, splitIndexes, comp, comp2, comp3,
            swapper);
    }
     ...
     * it <i>synchronously</i> partitions the rows of the given matrix by the
     * values of the given matrix column; This is essentially the same as
     * partitioning a list of composite objects by some instance variable; In
     * other words, two entire rows of the matrix are swapped, whenever two
     ...
     * are swapped, whenever two column values indicate so.
     * <p>
     * Of course, the column must not be a column of a different matrix. More
     * formally, there must hold: <br>
     * There exists an <tt>i</tt> such that
     ...
     * <td align="left">
     * <p>
     * <tt>column = matrix.viewColumn(0);<br>
     * a = 0;<br>
    ...
    private static void xPartitionOld(ObjectMatrix2D matrix, ObjectMatrix1D column, int from, int to,
         ...
         *
         * if (splitFrom>splitTo) return; // nothing to do if (from>to) { // all
         * bins are empty from--; for (int i = splitFrom; i<=splitTo; )
         * splitIndexes[i++] = from; return; } // Choose a partition (pivot)
         * index, m // Ideally, the pivot should be the median, because a median
         * splits a list into two equal sized sublists. // However, computing
         * the median is expensive, so we use an approximation. int medianIndex;
         * if (splitFrom==splitTo) { // we don't really have a choice
         * medianIndex = splitFrom; } else { // we do have a choice int m =
         * (from+to) / 2; // Small arrays, middle element int len = to-from+1;
         * if (len > SMALL) { int l = from; int n = to; if (len > MEDIUM) { //
         * Big arrays, pseudomedian of 9 int s = len/8; l = med3(column, l, l+s,
         * l+2*s); m = med3(column, m-s, m, m+s); n = med3(column, n-2*s, n-s,
         * n); } m = med3(column, l, m, n); // Mid-size, pseudomedian of 3 } //
         * Find the splitter closest to the pivot, i.e. the splitter that best
         * splits the list into two equal sized sublists. medianIndex =
         * cern.mateba.Sorting.binarySearchFromTo(splitters,column.getQuick(m),splitFrom,splitTo);
         * if (medianIndex < 0) medianIndex = -medianIndex - 1; // not found if
         * (medianIndex > splitTo) medianIndex = splitTo; // not found, one past
         * the end } splitter = splitters[medianIndex]; // Partition the list
         * according to the splitter, i.e. // Establish invariant: list[i] <
         * splitter <= list[j] for i=from..medianIndex and j=medianIndex+1 .. to
         * int splitIndex = xPartitionOld(matrix,column,from,to,splitter);
         * splitIndexes[medianIndex] = splitIndex; // Optimization: Handle
         * special cases to cut down recursions. if (splitIndex < from) { // no
         * element falls into this bin // all bins with splitters[i] <= splitter
         * are empty int i = medianIndex-1; while (i>=splitFrom && (!(splitter <
         * splitters[i]))) splitIndexes[i--] = splitIndex; splitFrom =
         * medianIndex+1; } else if (splitIndex >= to) { // all elements fall
         * into this bin // all bins with splitters[i] >= splitter are empty int
         * i = medianIndex+1; while (i<=splitTo && (!(splitter >
         * splitters[i]))) splitIndexes[i++] = splitIndex; splitTo =
         * medianIndex-1; } // recursively partition left half if (splitFrom <=
         * medianIndex-1) { xPartitionOld(matrix, column, from, splitIndex,
         * splitters, splitFrom, medianIndex-1, splitIndexes); } // recursively
         * partition right half if (medianIndex+1 <= splitTo) {
         * xPartitionOld(matrix, column, splitIndex+1, to, splitters,
         * medianIndex+1, splitTo, splitIndexes); }
         */
    }

    /**
     * Same as {@link #partition(int[], int, int, int)} except that it
     * <i>synchronously</i> partitions the rows of the given matrix by the
     * values of the given matrix column; This is essentially the same as
     * partitioning a list of composite objects by some instance variable; In
     * other words, two entire rows of the matrix are swapped, whenever two
     * column values indicate so.
    ...
    private static int xPartitionOld(ObjectMatrix2D matrix, ObjectMatrix1D column, int from, int to, Object splitter) {
        /*
         * Object element; // int, Object --> template type dependent for (int
         * i=from-1; ++i<=to; ) { element = column.getQuick(i); if (element <
         * splitter) { // swap x[i] with x[from] matrix.swapRows(i,from);
         * from++; } } return from-1;
         */
        return 0;
    }
