...
public class ParallelQuickSort {
    ...
    public static void quickSort(final byte[] x, final int off, int len, final ByteComparator comp, final int nThreads) {
        ...
        if (nThreads > 1) {
            ...
            if ((s = b - a) > 1) {
                final int s_f = s;
                other = ConcurrencyUtils.submit(() -> quickSort(x, off, s_f, comp, nThreads / 2));
            }
            if ((s = d - c) > 1) {
                if (other == null) {
                    final int s_f = s;
                    final int ns_f = n - s;
                    other = ConcurrencyUtils.submit(() -> quickSort(x, ns_f, s_f, comp, nThreads / 2));
                } else {
    ...
    public static void quickSort(final Object[] x, final int off, int len, final int nThreads) {
        ...
        if (nThreads > 1) {
            ...
            if ((s = b - a) > 1) {
                final int s_f = s;
                other = ConcurrencyUtils.submit(() -> quickSort(x, off, s_f, nThreads / 2));
            }
            if ((s = d - c) > 1) {
                if (other == null) {
                    final int s_f = s;
                    final int ns_f = n - s;
                    other = ConcurrencyUtils.submit(() -> quickSort(x, ns_f, s_f, nThreads / 2));
                } else {
