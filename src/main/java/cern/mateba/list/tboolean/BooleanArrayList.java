 ...
 */
package cern.mateba.list.tboolean;

import cern.mateba.Arrays;
import cern.mateba.buffer.tboolean.BooleanBuffer1DConsumer;
import cern.mateba.function.tboolean.BooleanProcedure;
...
/**
 * Resizable list holding {@code boolean} elements; implemented with arrays.
 */
@SuppressWarnings("unused")
// fixme reduce the number of abstractions?
// fixme remove duplicates in all classes
// todo check the approach with caching for performance
public class BooleanArrayList extends AbstractList implements BooleanBuffer1DConsumer {

    @Serial
    private static final long serialVersionUID = -6159052699587635342L;
    ...
    /**
     * Constructs an empty list with 10 elements by default.
     */
     ...
     */
    @Override
    public @NotNull BooleanArrayList clone() {
        val clone = (BooleanArrayList) super.clone();
     ...
     */
    public BooleanArrayList(final boolean @NotNull [] elements) {
        elements(elements);
    }
    ...
    /**
     * Sorts the specified range of the receiver into ascending numerical order ({@code false < true}).
     * <p>
     * The sorting algorithm is a count sort. This algorithm offers guaranteed O(n) performance without auxiliary
     * memory.
     *
     ...
     */
    public boolean @NotNull [] elements() {
        return elements;
    }
     ...
     */
    public boolean getQuick(final int index) {
        return elements[index];
    }

    /**
     * Sorts the specified range of the receiver into ascending order ({@code false < true}).
     * <p>
     ...
     */
    @Override
    public @NotNull BooleanArrayList partFromTo(final int from, final int to) {
        if (size == 0) return new BooleanArrayList(0);

        checkRangeFromTo(from, to, size);

        val part = new boolean[to - from + 1];
     ...
     */
    public void setQuick(final int index, final boolean element) {
     ...
     */
    protected static int toInt(final boolean value) {
        return value ? 1 : 0;
    }
     ...
     */
    public void beforeInsert(final int index, final boolean element) {
     ...
     *
     * @see #binarySearchFromTo(boolean, int, int)
     */
    public int binarySearch(final boolean key) {
     ...
     * than the specified key. Note that this guarantees that the return value will be {@code >= 0} if and only if the
     * key is found.
     */
    public int binarySearchFromTo(final boolean key, final int from, final int to) {
        int low = from;
        int high = to;
     ...
     */
    public boolean contains(final boolean elem) {
     ...
     */
    public void delete(final boolean element) {
     ...
     */
    public @NotNull BooleanArrayList elements(final boolean @NotNull [] elements) {
        clear();
        addAllOfFromTo(new BooleanArrayList(elements), 0, elements.length - 1); // fixme
        return this;
    }
    ...
    public boolean equals(final @Nullable Object otherObj) {
        ...
        if (!(otherObj instanceof BooleanArrayList other)) return false;
     ...
     */
    public void fillFromToWith(final int from, final int to, final boolean val) {
     ...
     */
    public boolean forEach(final @NotNull BooleanProcedure procedure) {
     ...
     * not found.
     */
    public int indexOfFromTo(final boolean element, final int from, final int to) {
        checkRangeFromTo(from, to, size);
     ...
     *
     * @param element the element to be searched for.
     * @return the index of the last occurrence of the element in the receiver; {@code -1} if the element is not found.
     */
     ...
     * @return the index of the last occurrence of the element in the receiver; {@code -1} if the element is not found.
     */
    public int lastIndexOfFromTo(final boolean element, final int from, final int to) {
        checkRangeFromTo(from, to, size());
     ...
     */
    public boolean removeAll(final @NotNull BooleanArrayList other) {
     ...
     * {@inheritDoc}
     *
     * @throws IllegalArgumentException when {@code other} is not an instance of {@link BooleanArrayList}.
     */
    @Override
    public void replaceFromToWithFrom(int from, int to, final @NotNull AbstractList other, int otherFrom) {
        if (other instanceof BooleanArrayList s) {
            int length = to - from + 1;
            if (length > 0) {
                checkRangeFromTo(from, to, size());
                checkRangeFromTo(otherFrom, otherFrom + length - 1, other.size());
                if (from <= otherFrom) while (--length >= 0)
                    setQuick(from++, s.getQuick(otherFrom++));
                else {
                    int otherTo = otherFrom + length - 1;
                    while (--length >= 0) setQuick(to--, s.getQuick(otherTo--));
                }
            }
    ...
    public void replaceFromWith(final int from, final @NotNull Collection<?> other) {
        ...
        int index = from;
        val limit = Math.min(size() - from, other.size());
        for (int i = 0; i < limit; i++) set(index++, (Boolean) e.next());
    }
     ...
     * @return {@code true} if the receiver changed as a result of the call.
     */
    public boolean retainAll(final @NotNull BooleanArrayList other) {
        if (other.size() == 0) {
     ...
     */
    public @NotNull BooleanArrayList times(final int times) {
        val newList = new BooleanArrayList(times * size());
        for (int i = times; --i >= 0; ) newList.addAllOfFromTo(this, 0, size() - 1);
        return newList;
    }
     ...
     */
    @Override
    public @NotNull ArrayList<Boolean> toList() {
        val mySize = size();
        val list = new ArrayList<Boolean>(mySize);
     ...
     */
    @Override
    public void addAllOf(@NotNull BooleanArrayList other) {
