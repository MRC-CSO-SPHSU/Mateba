 ...
 */
package cern.mateba.list.tchar;

import cern.mateba.Arrays;
import cern.mateba.Sorting;
import cern.mateba.buffer.tchar.CharBuffer1DConsumer;
import cern.mateba.function.tchar.CharComparator;
import cern.mateba.function.tchar.CharProcedure;
...
/**
 * Resizable list holding {@code char} elements; implemented with arrays.
 */
@SuppressWarnings("unused")
public class CharArrayList extends AbstractList implements CharBuffer1DConsumer {

    @Serial
    private static final long serialVersionUID = -4521314436300407036L;
     ...
     */
    public CharArrayList(final char @NotNull [] elements) {
        elements(elements);
    }
     ...
     * found.
     */
    public int binarySearchFromTo(final char key, final int from, final int to) {
     ...
     */
    protected void countSortFromTo(final int from, final int to, final char min, final char max) {
        if (size == 0) return;
     ...
     */
    public @NotNull CharArrayList elements(final char @NotNull [] elements) {
    ...
    public boolean equals(final @Nullable Object otherObj) {
        ...
        if (!(otherObj instanceof CharArrayList other)) return false;
     ...
     * @param index index of element to return.
     */
    public char getQuick(final int index) {
        return elements[index];
    }
     ...
     * @return a new list
     */
    public @NotNull CharArrayList partFromTo(final int from, final int to) {
        if (size == 0) return new CharArrayList(0);

        checkRangeFromTo(from, to, size);

        val part = new char[to - from + 1];
     ...
     */
    public void set(final int index, final char element) {
        checkRange(index, size);
     ...
     * @param element element to be stored at the specified position.
     */
    public void setQuick(final int index, final char element) {
        elements[index] = element;
    }
     ...
     *
     * @param index   index before which the specified element is to be inserted {@code (must be in [0,size]}).
     * @param element element to be inserted.
     */
    public void beforeInsert(final int index, final char element) {
     ...
     *
     * @see #binarySearchFromTo(char, int, int)
     */
     ...
     */
    @Override
    public @NotNull CharArrayList clone() {
        val clone = (CharArrayList) super.clone();
     ...
     */
    public void delete(final char element) {
     ...
     */
    public void fillFromToWith(final int from, final int to, final char val) {
     ...
     */
    public boolean forEach(final @NotNull CharProcedure procedure) {
     ...
     * not found.
     */
    public int indexOfFromTo(final char element, final int from, final int to) {
        checkRangeFromTo(from, to, size);
     ...
     * found.
     */
    public int lastIndexOfFromTo(final char element, final int from, final int to) {
        checkRangeFromTo(from, to, size());
     ...
     */
    public void mergeSortFromTo(final int from, final int to, final @NotNull CharComparator c) {
        val mySize = size();
        checkRangeFromTo(from, to, mySize);
     ...
     * @param c    the comparator to determine the order of the receiver.
     */
    public void quickSortFromTo(final int from, final int to, final @NotNull CharComparator c) {
        val mySize = size();
        checkRangeFromTo(from, to, mySize);
     ...
     */
    public boolean removeAll(final @NotNull CharArrayList other) {
     ...
     * {@inheritDoc}
     *
     * @throws IllegalArgumentException when {@code other} is not an instance of {@link CharArrayList}.
     */
    @Override
    public void replaceFromToWithFrom(int from, int to, final @NotNull AbstractList other, int otherFrom) {
        if (other instanceof CharArrayList s) {
     ...
     * @return {@code true} if the receiver changed as a result of the call.
     */
    public boolean retainAll(final @NotNull CharArrayList other) {
        if (other.size() == 0) {
     ...
     */
    public CharArrayList times(final int times) {
        val newList = new CharArrayList(times * size());
     ...
     */
    @Override
    public @NotNull ArrayList<Character> toList() {
        val mySize = size();
        val list = new ArrayList<Character>(mySize);
     ...
     */
    @Override
    public void addAllOf(final @NotNull CharArrayList other) {
