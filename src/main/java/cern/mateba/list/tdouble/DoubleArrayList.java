...
import cern.mateba.function.tdouble.DoubleComparator;
...
/**
 * Resizable list holding {@code double} elements; implemented with arrays.
 */
@SuppressWarnings("unused")
public class DoubleArrayList extends AbstractList implements DoubleBuffer1DConsumer {

    @Serial
    private static final long serialVersionUID = -1532821626651951625L;
     ...
     */
    public DoubleArrayList(final double @NotNull [] elements) {
        elements(elements);
    }
     ...
     * is found.
     */
    public int binarySearchFromTo(final double key, final int from, final int to) {
     ...
     */
    @Override
    public @NotNull DoubleArrayList clone() {
        val clone = (DoubleArrayList) super.clone();
    ...
    public double @NotNull [] elements() {
        return elements;
    }
     ...
     */
    public @NotNull DoubleArrayList elements(final double @NotNull [] elements) {
    ...
    public boolean equals(final @Nullable Object otherObj) {
        ...
        if (!(otherObj instanceof DoubleArrayList other)) return false;
     ...
     * <b>You should only use this method when you are absolutely sure that the index is within bounds.</b> Precondition
     ...
     * @return a new list
     */
    public @NotNull DoubleArrayList partFromTo(final int from, final int to) {
        if (size == 0) return new DoubleArrayList(0);

        checkRangeFromTo(from, to, size);

        val part = new double[to - from + 1];
     ...
     * @param element element to be stored at the specified position.
     */
    public void setQuick(final int index, final double element) {
        elements[index] = element;
    }
     ...
     */
    @Override
    public void addAllOf(final @NotNull DoubleArrayList other) {
     ...
     */
    public void beforeInsert(final int index, final double element) {
     ...
     */
    public boolean contains(final double elem) {
     ...
     */
    public void delete(final double element) {
     ...
     */
    public void fillFromToWith(final int from, final int to, final double val) {
     ...
     * not found.
     */
    public int indexOfFromTo(final double element, final int from, final int to) {
        checkRangeFromTo(from, to, size);
     ...
     */
    public void mergeSortFromTo(final int from, final int to, final @NotNull DoubleComparator c) {
        val mySize = size();
        checkRangeFromTo(from, to, mySize);
     ...
     * @param c    the comparator to determine the order of the receiver.
     */
    public void quickSortFromTo(final int from, final int to, final DoubleComparator c) {
        val mySize = size();
        checkRangeFromTo(from, to, mySize);
     ...
     */
    public boolean removeAll(final @NotNull DoubleArrayList other) {
    ...
    public void replaceFromToWithFrom(int from, int to, final @NotNull AbstractList other, int otherFrom) {
        if (other instanceof DoubleArrayList s) {
    ...
    public void replaceFromWith(final int from, final @NotNull Collection<?> other) {
        ...
        for (int i = 0; i < limit; i++) set(index++, ((Number) e.next()).doubleValue());
    }
     ...
     * @return {@code true} if the receiver changed as a result of the call.
     */
    public boolean retainAll(final @NotNull DoubleArrayList other) {
        if (other.size() == 0) {
    ...
    public void shuffleFromTo(final int from, final int to) {
        ...
        val gen = new Uniform(AbstractDistribution.makeDefaultGenerator());
        int random;
        double tmpElement;
     ...
     */
    public @NotNull DoubleArrayList times(final int times) {
        val newList = new DoubleArrayList(times * size());
     ...
     */
    @Override
    public @NotNull ArrayList<Double> toList() {
        val mySize = size();
        val list = new ArrayList<Double>(mySize);
