...
import cern.mateba.buffer.tlong.LongBuffer1DConsumer;
...
/**
 * Resizable list holding {@code long} elements; implemented with arrays.
 */
@SuppressWarnings("unused")
public class LongArrayList extends AbstractList implements LongBuffer1DConsumer {

    @Serial
    private static final long serialVersionUID = 2350006671236186580L;
     ...
     */
    public LongArrayList(final long @NotNull [] elements) {
        elements(elements);
    }
     ...
     * found.
     */
    public int binarySearchFromTo(final long key, final int from, final int to) {
     ...
     */
    @Override
    public @NotNull LongArrayList clone() {
        val clone = (LongArrayList) super.clone();
     ...
     */
    protected void countSortFromTo(final int from, final int to, final long min, final long max) {
        if (size == 0 || size == 1) return;
        checkRangeFromTo(from, to, size);

        val width = (int) (max - min + 1);
     ...
     */
    public @NotNull LongArrayList elements(final long @NotNull [] elements) {
     ...
     * @return a new list
     */
    public @NotNull LongArrayList partFromTo(final int from, final int to) {
        if (size == 0) return new LongArrayList(0);

        checkRangeFromTo(from, to, size);

        val part = new long[to - from + 1];
     ...
     * @param element element to be stored at the specified position.
     */
    public void setQuick(final int index, final long element) {
        elements[index] = element;
    }
     ...
     */
    public void beforeInsert(final int index, final long element) {
     ...
     * otherwise, {@code -insertion_point - 1}. The <i>insertion point</i> is defined as the point at which the value
     * would be inserted into the receiver: the index of the first element greater than the key, or
     * {@code receiver.size()}, if all elements in the receiver are less than the specified key. Note that this
     ...
     */
    public boolean contains(final long elem) {
     ...
     */
    public void delete(final long element) {
    ...
    public boolean equals(final @Nullable Object otherObj) {
        ...
        if (!(otherObj instanceof LongArrayList other)) return false;
     ...
     */
    public void fillFromToWith(final int from, final int to, final long val) {
     ...
     */
    public boolean forEach(final @NotNull LongProcedure procedure) {
     ...
     * not found.
     */
    public int indexOfFromTo(final long element, final int from, final int to) {
        checkRangeFromTo(from, to, size);
     ...
     * found.
     */
    public int lastIndexOfFromTo(final long element, final int from, final int to) {
        checkRangeFromTo(from, to, size());
     ...
     */
    public void mergeSortFromTo(final int from, final int to, final @NotNull LongComparator c) {
        val mySize = size();
        checkRangeFromTo(from, to, mySize);
    ...
    public void quickSortFromTo(final int from, final int to) {
        ...
        val myElements = elements();
        cern.mateba.Sorting.parallelQuickSort(myElements, from, to + 1, Long::compare);
        elements(myElements);
        setSizeRaw(mySize);
    }
     ...
     * @param c    the comparator to determine the order of the receiver.
     */
    public void quickSortFromTo(final int from, final int to, final @NotNull LongComparator c) {
        val mySize = size();
        checkRangeFromTo(from, to, mySize);

        val myElements = elements();
        Sorting.parallelQuickSort(myElements, from, to + 1, c);
        elements(myElements);
        setSizeRaw(mySize);
    }
     ...
     */
    public boolean removeAll(final @NotNull LongArrayList other) {
    ...
    public void replaceFromToWithFrom(int from, int to, final @NotNull AbstractList other, int otherFrom) {
        if (other instanceof LongArrayList s) {
     ...
     * @return {@code true} if the receiver changed as a result of the call.
     */
    public boolean retainAll(final @NotNull LongArrayList other) {
        if (other.size() == 0) {
     ...
     */
    public void shuffleFromTo(final int from, final int to) {// fixme this should be abstract and and overridden everywhere
        checkRangeFromTo(from, to, size());
        int random;
        long tmpElement;
        val gen = new Uniform(new MersenneTwister(new java.util.Date()));
        for (int i = from; i < to; i++) {
            random = gen.nextIntFromTo(i, to);// fixme validate how this works with the new generator
    ...
    public int size() { // fixme remove size everywhere
        return size;
    }
     ...
     */
    public @NotNull LongArrayList times(final int times) {
        val newList = new LongArrayList(times * size());
     ...
     */
    @Override
    public @NotNull ArrayList<Long> toList() {
        val mySize = size();
        val list = new ArrayList<Long>(mySize);
     ...
     */
    @Override
    public void addAllOf(final @NotNull LongArrayList other) {
