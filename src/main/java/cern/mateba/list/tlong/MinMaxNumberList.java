...
import cern.mateba.matrix.tbit.BitVector;
import cern.mateba.matrix.tbit.QuickBitVector;
 ...
 * implemented with a <tt>cern.mateba.bitvector.BitVector</tt>. First see the <a
...
public class MinMaxNumberList extends LongArrayList {
    ...
    public static int bitsPerElement(long minimum, long maximum) {
        ...
        if (1 + maximum - minimum > 0) {
            bits = (int) Math.round(Math.ceil(cern.jet.math.tdouble.DoubleArithmetic.log(2, 1 + maximum - minimum)));
        } else {
            // overflow or underflow in calculating "1+maximum-minimum"
            // happens if signed long representation is too short for doing
            // unsigned calculations
            // e.g. if minimum==LONG.MIN_VALUE, maximum==LONG.MAX_VALUE
            // --> in such cases store all bits of values without any
            // compression.
            bits = 64;
        }
        return bits;
    ...
    public void partFromTo(final int from, final int to, final BitVector qualificants, final int qualificantsFrom,
        ...
        // BitVector tmpBitVector = new BitVector(this.bits,
        // this.size*bitsPerElem);
        for (int i = from; i <= to; i++, q++, p++, j += bitsPerElem) {
            if (qualificants == null || qualificants.get(q)) {
                // part[p] = minVal + tmpBitVector.getLongFromTo(j,
                // j+bitsPerElem-1);
