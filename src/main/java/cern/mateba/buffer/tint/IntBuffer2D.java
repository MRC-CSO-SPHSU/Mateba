...
/**
 * Fixed sized (non-resizable) streaming buffer connected to a target {@link IntBuffer2DConsumer} to which data is
 * automatically flushed upon buffer overflow.
 */
public class IntBuffer2D implements IntBuffer2DConsumer, Serializable, Cloneable {

    @Serial
    private static final long serialVersionUID = -6566046230959061245L;
     ...
     *                 the target.
     */
    public IntBuffer2D(final @NotNull IntBuffer2DConsumer target, final int capacity) {
        this.target = target;
     ...
     */
    public void addAllOf(final @NotNull IntArrayList x, final @NotNull IntArrayList y) {
        val listSize = x.size();
     ...
     */
    @Override
    public @NotNull IntBuffer2D clone() {
        try {
            val clone = (IntBuffer2D) super.clone();
