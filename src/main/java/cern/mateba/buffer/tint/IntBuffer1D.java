 ...
 */
package cern.mateba.buffer.tint;
...
/**
 * Fixed sized (non-resizable) streaming buffer connected to a target {@link IntBuffer1DConsumer} to which data is
 * automatically flushed upon buffer overflow.
 */
public class IntBuffer1D implements IntBuffer1DConsumer, Serializable, Cloneable {

    @Serial
    private static final long serialVersionUID = -1457967324592562957L;
    protected IntBuffer1DConsumer target;
     ...
     *                 the target.
     */
    public IntBuffer1D(final @NotNull IntBuffer1DConsumer target, final int capacity) {
        this.target = target;
     ...
     */
    public void addAllOf(final @NotNull IntArrayList list) {
     ...
     */
    @Override
    public @NotNull IntBuffer1D clone() {
        try {
            val clone = (IntBuffer1D) super.clone();
