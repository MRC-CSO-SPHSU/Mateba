...
/**
 * Fixed sized (non-resizable) streaming buffer connected to a target {@link CharBuffer3DConsumer} to which data is
 * automatically flushed upon buffer overflow.
 */
public class CharBuffer3D implements CharBuffer3DConsumer, Serializable, Cloneable {

    @Serial
    private static final long serialVersionUID = 1158062051278494781L;
     ...
     *                 the target.
     */
    public CharBuffer3D(final @NotNull CharBuffer3DConsumer target, final int capacity) {
        this.target = target;
     ...
     */
    public void add(final char x, final char y, final char z) {
        if (this.size == this.capacity) flush();
        this.xElements[this.size] = x;
        this.yElements[this.size++] = y;
        this.zElements[this.size++] = z;// todo check this indexation
    }
     ...
     */
    public void addAllOf(final @NotNull CharArrayList xElements, final @NotNull CharArrayList yElements,
                         final @NotNull CharArrayList zElements) {
     ...
     */
    @Override
    public @NotNull CharBuffer3D clone() {
        try {
            val clone = (CharBuffer3D) super.clone();
