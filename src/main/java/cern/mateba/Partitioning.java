...
public class Partitioning {
    ...
    public static void dualPartition(double[] list, double[] secondary, int from, int to, double[] splitters,
        ...
        // Ideally, the pivot should be the median, because a median splits a
        // list into two equal sized sublists.
        // However, computing the median is expensive, so we use an
        // approximation.
        int medianIndex;
        if (splitFrom == splitTo) { // we don't really have a choice
            ...
            // Find the splitter closest to the pivot, i.e. the splitter that
            // best splits the list into two equal sized sublists.
            medianIndex = Sorting.binarySearchFromTo(splitters, list[m], splitFrom, splitTo);
            if (medianIndex < 0)
                medianIndex = -medianIndex - 1; // not found
            if (medianIndex > splitTo)
                medianIndex = splitTo; // not found, one past the end

        }
        splitter = splitters[medianIndex];

        // Partition the list according to the splitter, i.e.
        // Establish invariant: list[i] < splitter <= list[j] for
        // i=from..medianIndex and j=medianIndex+1 .. to
     ...
     * user provided {@link cern.mateba.Swapper} object that knows how to swap the
    ...
    public static void partition(int[] list, int from, int to, int[] splitters, int splitFrom, int splitTo,
        ...
        if (splitFrom == splitTo) { // we don't really have a choice
            ...
            // int key = list[m];
            /*
             * if (splitTo-splitFrom+1 < 5) { // on short lists linear search is
             * quicker int i=splitFrom-1; while (++i <= splitTo && list[i] <
             * key); if (i > splitTo || list[i] > key) i = -i-1; // not found
             * medianIndex = i; }
             */
            // else {
            /*
             ...
             * mid; // key found } medianIndex = mid; if (low > high)
             * medianIndex = -(medianIndex + 1); // key not found. //}
             */
        ...
        /*
         * steps += to-from+1; int head = from; for (int i=from-1; ++i<=to; ) { //
         * swap all elements < splitter to front element = list[i]; if (element <
         * splitter) { list[i] = list[head]; list[head++] = element;
         * //swappedElements++; } } int splitIndex = head-1;
         */
        ...
        // recursively partition left half
        if (splitFrom <= medianIndex - 1) {
            // System.out.println("1.recursive: from="+from+",
            // to="+splitIndex+", splitFrom="+splitFrom+",
            // splitTo="+(medianIndex-1));
            partition(list, from, splitIndex, splitters, splitFrom, medianIndex - 1, splitIndexes);
        }
        ...
        // recursively partition right half
        if (medianIndex + 1 <= splitTo) {
            // System.out.println("2.recursive: from="+(splitIndex+1)+",
            // to="+to+", splitFrom="+(medianIndex+1)+", splitTo="+splitTo);
            partition(list, splitIndex + 1, to, splitters, medianIndex + 1, splitTo, splitIndexes);
        }
    ...
    public static int partition(int[] list, int from, int to, int splitter) {
        ...
        /*
         * System.out.println(); if (from<=to) { System.out.println("SORT
         * WORKING: from="+from+", to="+to+", splitter="+splitter); } else {
         * System.out.println("SORT WORKING: NOTHING TO DO."); }
         */

        // returns index of last element < splitter
        /*
         * for (int i=from-1; ++i<=to; ) { if (list[i] < splitter) { int
         * element = list[i]; list[i] = list[from]; list[from++] = element; } }
         */
        ...
        /*
         * //JAL: int first = from; int last = to+1; --first; while (true) {
         * while (++first < last && list[first] < splitter); while (first <
         * --last && !(list[last] < splitter)); if (first >= last) return
         * first-1; int tmp = list[first]; list[first] = list[last]; list[last] =
         * tmp; }
         */
        ...
        /*
         * //System.out.println("splitter="+splitter);
         * //System.out.println("before="+new IntArrayList(list)); to++; //int
         * head = from; int element; //int oldHead; while (--to >= from) {
         * element = list[to]; if (element < splitter) { from--; while (++from <
        ...
        /*
         * int i=from-1; int head = from; int trail = to; while (++i <= trail) {
         * int element = list[i]; if (element < splitter) { if (head == i)
         * head++; else { // swap list[i] with list[from] int oldHead =
         * list[head]; int oldTrail = list[trail]; list[head++] = element;
         * list[i--] = oldTrail; list[trail--] = oldHead; } }
