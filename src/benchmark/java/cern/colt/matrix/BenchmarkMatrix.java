 ...
 * href="../doc-files/performanceLog.html">performance log</a>.
 * 
...
public class BenchmarkMatrix {
    ...
    protected static Double2DProcedure fun_dgemm(final boolean transposeA, final boolean transposeB) {
        return new Double2DProcedure() {
            ...
            public void apply(cern.colt.Timer timer) {
                ...
                smpBlas.dgemm(transposeA, transposeB, 1, A, B, 0, C);
            }
    ...
    protected static Double2DProcedure fun_dgemv(final boolean transposeA) {
        return new Double2DProcedure() {
            ...
            public void apply(cern.colt.Timer timer) {
                DoubleBlas smpBlas = new SmpDoubleBlas();
                smpBlas.dgemv(transposeA, 1, A, B.viewRow(0), 0, C.viewRow(0));
            }
    ...
    protected static Double2DProcedure funAssignGetSet() {
        return new Double2DProcedure() {
            ...
            public void apply(cern.colt.Timer timer) {
                ...
                /*
                 * for (int row=rows; --row >= 0; ) { for (int column=columns;
                 * --column >= 0; ) { A.set(row,column, B.get(row,column)); } }
                 */
    ...
    protected static Double2DProcedure funCorrelation() {
        return new Double2DProcedure() {
            ...
            public void setParameters(DoubleMatrix2D A, DoubleMatrix2D B) {
                super.setParameters(A.viewDice(), B); // transposed --> faster
                // (memory aware)
                // iteration in
                // correlation algo
            }
    ...
    protected static Double2DProcedure funSetQuick() {
        return new Double2DProcedure() {
            ...
            public void apply(cern.colt.Timer timer) {
                ...
                for (int row = 0; row < rows; row++) {
                    for (int column = 0; column < columns; column++) {
                        // a very fast random number generator (this is an
                        // inline version of class cern.jet.random.engine.DRand)
                        current *= 0x278DDE6D;
    ...
    protected static Double2DProcedure getGenericFunction(String cmd) {
        if (cmd.equals("dgemm"))
            ...
            return funAssign();
        else if (cmd.equals("assignGetSet"))
            return funAssignGetSet();
        else if (cmd.equals("assignGetSetQuick"))
            return funAssignGetSetQuick();
        else if (cmd.equals("elementwiseMult"))
            return funElementwiseMult();
        else if (cmd.equals("elementwiseMultB"))
            return funElementwiseMultB();
        else if (cmd.equals("SOR5"))
            return funSOR5();
        else if (cmd.equals("SOR8"))
            return funSOR8();
        else if (cmd.equals("LUDecompose"))
            return funLUDecompose();
        else if (cmd.equals("LUSolve"))
            return funLUSolve();
        else if (cmd.equals("assignLog"))
            return funAssignLog();
        else if (cmd.equals("assignPlusMult"))
    ...
    /**
     * Runs the matrix benchmark operations defined in args or in the file
     * specified by args0. To get detailed help on usage type java
     * cern.colt.matrix.bench.BenchmarkMatrix -help
     */
    public static void main(String[] args) {
        ...
        System.out.println(BenchmarkMatrixKernel.systemInfo() + "\n");
        ...
        if (!args[0].equals("-file")) { // interactive mode, commands supplied
            ...
            /*
             * parse command file in args[0] one command per line (including
             * parameters) for example: // dgemm dense 2 2.0 false true 0.999 10
             * 30 50 100 250 500 1000 dgemm dense 2 2.5 false true 0.999 10 50
            ...
            try {
                ...
                while ((token = stream.nextToken()) != stream.TT_EOF) { // while
                    ...
                    // of
                    // file
                    if (token == stream.TT_EOL) { // execute a command line at
                        // a time
                ...
                /*
                 * java.io.InputStream input = new java.io.DataInputStream(new
                 * java.io.BufferedInputStream(new
                 * java.io.FileInputStream(args[1]))); BufferedReader d = new
                 * BufferedReader(new InputStreamReader(in));
                 */
                String line;
                while ((line = reader.readLine()) != null) { // while not end
                    // of file
    ...
    protected static void run(double minSeconds, String title, Double2DProcedure function, String[] types, int[] sizes,
        ...
        for (int k = 0; k < types.length; k++) {
            // DoubleFactory2D factory = (k==0 ? DoubleFactory2D.dense : k==1 ?
            // DoubleFactory2D.sparse : DoubleFactory2D.rowCompressed);
            // DoubleFactory2D factory = (k==0 ? DoubleFactory2D.dense : k==1 ?
            // DoubleFactory2D.sparse : k==2 ? DoubleFactory2D.rowCompressed :
            // DoubleFactory2D.rowCompressedModified);
            ...
            for (int i = 0; i < sizes.length; i++) {
                ...
                for (int j = 0; j < densities.length; j++) {
                    ...
                    // if (true) {
                    // if (!((k==1 && density >= 0.1 && size >=100) ||
                    // (size>5000 && (k==0 || density>1.0E-4) ))) {
                    if (!((k > 0 && density >= 0.1 && size >= 500))) {
                        double val = 0.5;
                        function.A = null;
                        function.B = null;
                        function.C = null;
                        function.D = null; // --> help gc before allocating new
                        // mem
                        ...
                        double secs = BenchmarkMatrixKernel.run(minSeconds, function);
                        opsPerSec = (float) (ops / secs);
                    } else { // skip this parameter combination (not used in
                        // practice & would take a lot of memory and
                        // time)
                        opsPerSec = Float.NaN;
                    }
        ...
        for (int j = densities.length; --j >= 0;)
         ...
         * System.out.println("\n"+new
         * cern.colt.matrix.doublealgo.Formatter("%1.3G").toTitleString(speedup,rowNames,colNames,rowAxisName,colAxisName,title,aggr));
         */
    ...
    protected static void runSpecial(double minSeconds, String title, Double2DProcedure function) {
        ...
        for (int i = 0; i < sizes.length; i++) {
            ...
            function.B = null;
            function.C = null;
            function.D = null; // --> help gc before allocating new mem
            DoubleMatrix2D A = factory.sample(size, size, val, density);
            DoubleMatrix2D B = factory.sample(size, size, val, density);
            function.setParameters(A, B);
            A = null;
            B = null; // help gc
            float secs = BenchmarkMatrixKernel.run(minSeconds, function);
            double ops = function.operations();
    ...
    protected static String usage(String cmd) {
        ...
        // String usage = "Illegal arguments! Arguments to be supplied:\n" +
                // "\te.g. "+cmd+" dense 2 2.0 false 0.999 10 30 50 100 250 500
                // 1000\n"+
