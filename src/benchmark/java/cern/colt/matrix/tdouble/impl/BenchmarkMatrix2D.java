...
/**
 * Benchmarks the performance of matrices. Here are the results of some
 * encouraging measurements. Note that all benchmarks only measure the time
 * spent in accessing a matrix element; they exclude the loop itself.
 * <p>
 * <center>
...
class BenchmarkMatrix2D {
    ...
    public static void doubleBenchmark(int runs, int rows, int columns, String kind, boolean print,
        ...
        emptyLoop.stop();
        System.out.println(dummy); // !!! so that the jitter can't optimize
        // away the whole loop
        ...
        emptyLoop2.stop();
        System.out.println(dummy2); // !!! so that the jitter can't optimize
        // away the whole loop
        ...
        // else if (kind.equals("denseArray")) matrix = new
        // DoubleArrayMatrix2D(rows,columns);
        else
        ...
        /*
         * if (kind.equals("sparse")) { int hashCollisions =
         * ((SparseDoubleMatrix2D)matrix).elements.hashCollisions;
         * System.out.println("hashCollisions="+hashCollisions);
         * System.out.println("--> "+ ((double)hashCollisions / (rows*columns)) +"
         * hashCollisions/element on average."); }
         */
        ...
        if (print)
            System.out.println(matrix);
        // if (kind.equals("sparse"))
        // System.out.println("hashCollisions="+((SparseDoubleMatrix2D)matrix).elements.hashCollisions);
        System.out.println(element); // !!! so that the jitter can't optimize
        // away the whole loop
        ...
        // if (kind.equals("sparse"))
        // System.out.println("hashCollisions="+((SparseDoubleMatrix2D)view).elements.hashCollisions);
        System.out.println(element); // !!! so that the jitter can't optimize
        // away the whole loop
    ...
    public static void doubleBenchmarkMult(int runs, int rows, int columns, String kind, boolean print,
        ...
        for (int i = 0; i < runs; i++) {
            timer1.start();
            matrix.assign(DoubleFunctions.mult(3));
            timer1.stop();
        }
        timer1.display();
    ...
    public static void doubleBenchmarkPrimitive(int runs, int rows, int columns, boolean print) {
        ...
        for (int i = 0; i < runs; i++) {
            /*
             * // initializing for (int column=0; column < columns; column++) {
             * for (int row=0; row < rows; row++) {
             * matrix.setQuick(row,column,1); } }
             */
            timer3.start();
    ...
    public static void intBenchmark(int runs, int rows, int columns, String kind, boolean print, int initialCapacity,
        ...
        /*
         * // certain loops need to be constructed so that the jitter can't
         * optimize them away and we get fantastic numbers. // this involves
         * primarly read-loops
         * 
         * cern.colt.Timer timer1 = new cern.colt.Timer(); cern.colt.Timer
         * timer2 = new cern.colt.Timer(); cern.colt.Timer timer3 = new
         * cern.colt.Timer(); cern.colt.Timer emptyLoop = new cern.colt.Timer();
         * cern.colt.Timer emptyLoop2 = new cern.colt.Timer();
         * 
         * emptyLoop.start(); int dummy = 0; for (int i=0; i<runs; i++) { for
         * (int column=0; column < columns; column++) { for (int row=0; row <
         * rows; row++) { dummy++; } } } emptyLoop.stop();
         * System.out.println(dummy); // !!! so that the jitter can't optimize
         * away the whole loop
         * 
         * emptyLoop2.start(); dummy = 3; int dummy2 = 0; for (int i=0; i<runs;
         * i++) { for (int value = 0, column=0; column < columns; column++) {
         * for (int row=0; row < rows; row++) { dummy2 += dummy; } } }
         * emptyLoop2.stop(); System.out.println(dummy2); // !!! so that the
         * jitter can't optimize away the whole loop
         * 
         * long before = Runtime.getRuntime().freeMemory(); long size =
         * (((long)rows)*columns)*runs;
         * 
         * AbstractIntMatrix2D matrix = null; if (kind.equals("sparse")) matrix =
         * new
         * SparseIntMatrix2D(rows,columns,initialCapacity,minLoadFactor,maxLoadFactor);
         * else if (kind.equals("dense")) matrix = new
         * DenseIntMatrix2D(rows,columns); //else if (kind.equals("denseArray"))
         * matrix = new DoubleArrayMatrix2D(rows,columns); else throw new
         * RuntimeException("unknown kind");
         * 
         * System.out.println("\nNow filling..."); if (kind.equals("sparse"))
         * ((SparseIntMatrix2D)matrix).elements.hashCollisions = 0; for (int
         * i=0; i<runs; i++) { matrix.assign(0);
         * matrix.ensureCapacity(initialCapacity); if (kind.equals("sparse"))
         * ((SparseIntMatrix2D)matrix).ensureCapacity(initialCapacity);
         * timer1.start(); int value = 0; for (int column=0; column < columns;
         * column++) { for (int row=0; row < rows; row++) {
         * matrix.setQuick(row,column,value++); } } timer1.stop(); }
         * timer1.display(); timer1.minus(emptyLoop).display();
         * System.out.println(size / timer1.minus(emptyLoop).seconds() +"
         * elements / sec");
         * 
         * Runtime.getRuntime().gc(); // invite gc try {
         * Thread.currentThread().sleep(1000); } catch (InterruptedException
         * exc) {}; long after = Runtime.getRuntime().freeMemory();
         * System.out.println("KB needed="+(before-after) / 1024);
         * System.out.println("bytes needed per non-zero="+(before-after) /
         * (double)matrix.cardinality()); if (print) {
         * System.out.println(matrix); if (kind.equals("sparse"))
         * System.out.println("map="+((SparseIntMatrix2D)matrix).elements); } if
         * (kind.equals("sparse")) { int hashCollisions =
         * ((SparseIntMatrix2D)matrix).elements.hashCollisions;
         * System.out.println("hashCollisions="+hashCollisions);
         * System.out.println("--> "+ ((double)hashCollisions / (rows*columns)) +"
         * probes/element on average."); }
         * 
         * System.out.println("\nNow reading..."); if (kind.equals("sparse"))
         * ((SparseIntMatrix2D)matrix).elements.hashCollisions = 0;
         * timer2.start(); int element=0; for (int i=0; i<runs; i++) { for (int
         * column=0; column < columns; column++) { for (int row=0; row < rows;
         * row++) { element += matrix.getQuick(row,column); } } }
         * timer2.stop().display(); timer2.minus(emptyLoop2).display();
         * System.out.println(size / timer2.minus(emptyLoop2).seconds() +"
         * elements / sec"); if (print) System.out.println(matrix); if
         * (kind.equals("sparse"))
         * System.out.println("hashCollisions="+((SparseIntMatrix2D)matrix).elements.hashCollisions);
         * System.out.println(element); // !!! so that the jitter can't optimize
         * away the whole loop
         * 
         * System.out.println("\nNow removing..."); before =
         * Runtime.getRuntime().freeMemory(); if (kind.equals("sparse"))
         * ((SparseIntMatrix2D)matrix).elements.hashCollisions = 0; for (int
         * i=0; i<runs; i++) { // initializing for (int column=0; column <
         * columns; column++) { for (int row=0; row < rows; row++) {
         * matrix.setQuick(row,column,1); } } timer3.start(); for (int column=0;
         * column < columns; column++) { for (int row=0; row < rows; row++) {
         * matrix.setQuick(row,column,0); } } timer3.stop(); } timer3.display();
         * timer3.minus(emptyLoop).display(); System.out.println(size /
         * timer3.minus(emptyLoop).seconds() +" elements / sec");
         * Runtime.getRuntime().gc(); // invite gc try {
         * Thread.currentThread().sleep(1000); } catch (InterruptedException
         * exc) {}; after = Runtime.getRuntime().freeMemory();
         * System.out.println("KB needed="+(before-after)/1024);
         * System.out.println("KB free="+(after/1024));
         * 
         * if (print) System.out.println(matrix); if (kind.equals("sparse"))
         * System.out.println("hashCollisions="+((SparseIntMatrix2D)matrix).elements.hashCollisions);
         * 
         * 
         * System.out.println("bye bye.");
    ...
    public static void intBenchmarkPrimitive(int runs, int rows, int columns, boolean print) {
         ...
         * // certain loops need to be constructed so that the jitter can't
         * optimize them away and we get fantastic numbers. // this involves
         * primarly read-loops cern.colt.Timer timer1 = new cern.colt.Timer();
         * cern.colt.Timer timer2 = new cern.colt.Timer(); cern.colt.Timer
         * timer3 = new cern.colt.Timer(); cern.colt.Timer emptyLoop = new
         * cern.colt.Timer(); cern.colt.Timer emptyLoop2 = new
         * cern.colt.Timer();
         * 
         * emptyLoop.start(); int dummy = 0; for (int i=0; i<runs; i++) { for
         * (int column=0; column < columns; column++) { for (int row=0; row <
         ...
         * 
         * emptyLoop2.start(); dummy = 3; int dummy2 = 0; for (int i=0; i<runs;
         * i++) { for (int column=0; column < columns; column++) { for (int
         * row=0; row < rows; row++) { dummy2 += dummy; } } } emptyLoop2.stop();
         * System.out.println(dummy2); // !!! so that the jitter can't optimize
         * away the whole loop
         * 
         * long before = Runtime.getRuntime().freeMemory(); long size =
         ...
         * timer1.start(); int value = 0; for (int column=0; column < columns;
         * column++) { for (int row=0; row < rows; row++) { matrix[row][column] =
         * value++; } } timer1.stop(); } timer1.display();
         * timer1.minus(emptyLoop).display(); System.out.println(size /
         * timer1.minus(emptyLoop).seconds() +" elements / sec");
         * 
         * Runtime.getRuntime().gc(); // invite gc try {
         * Thread.currentThread().sleep(1000); } catch (InterruptedException
         * exc) {}; long after = Runtime.getRuntime().freeMemory();
         * System.out.println("KB needed="+(before-after) / 1024); if (print) {
         * DenseIntMatrix2D m = new DenseIntMatrix2D(rows,columns);
         * m.assign(matrix); System.out.println(m); }
         * 
         * System.out.println("\nNow reading..."); timer2.start(); int
         * element=0; for (int i=0; i<runs; i++) { for (int column=0; column <
         * columns; column++) { for (int row=0; row < rows; row++) { element +=
         * matrix[row][column]; } } } timer2.stop().display();
         * timer2.minus(emptyLoop2).display(); System.out.println(size /
         * timer2.minus(emptyLoop2).seconds() +" elements / sec"); if (print) {
         * DenseIntMatrix2D m = new DenseIntMatrix2D(rows,columns);
         * m.assign(matrix); System.out.println(m); }
         * System.out.println(element); // !!! so that the jitter can't optimize
         * away the whole loop
         * 
         * System.out.println("\nNow removing..."); before =
         * Runtime.getRuntime().freeMemory(); for (int i=0; i<runs; i++) {
         * timer3.start(); for (int column=0; column < columns; column++) { for
         * (int row=0; row < rows; row++) { matrix[row][column] = 0; } }
         * timer3.stop(); } timer3.display(); timer3.minus(emptyLoop).display();
         * System.out.println(size / timer3.minus(emptyLoop).seconds() +"
         * elements / sec"); Runtime.getRuntime().gc(); // invite gc try {
         * Thread.currentThread().sleep(1000); } catch (InterruptedException
         * exc) {}; after = Runtime.getRuntime().freeMemory();
         * System.out.println("KB needed="+(before-after)/1024);
         * System.out.println("KB free="+(after/1024));
         * 
         * if (print) { DenseIntMatrix2D m = new DenseIntMatrix2D(rows,columns);
         * m.assign(matrix); System.out.println(m); }
         * 
         * System.out.println("bye bye.");
         */
    ...
    public static void intBenchmarkPrimitiveOptimized(int runs, int rows, int columns, boolean print) {
         ...
         * away the whole loop
         * 
         * int[][] matrix = new int[rows][columns]; emptyLoop2.start(); dummy =
         * 3; int dummy2 = 7; System.out.println(dummy2); // !!! so that the
         * jitter can't optimize away the whole loop for (int i=0; i<runs; i++) {
         * for (int column=0; column < columns; column++) { for (int row=0; row <
         * rows; row++) { dummy2 += dummy; //matrix[row][column]; } } }
         * emptyLoop2.stop(); System.out.println(dummy2); // !!! so that the
         * jitter can't optimize away the whole loop
         * 
         * long before = Runtime.getRuntime().freeMemory(); long size =
         ...
         * 
         * System.out.println("\nNow filling..."); for (int i=0; i<runs; i++) {
         * timer1.start(); int value = 0; for (int row=0; row < rows; row++) {
         * int[] r = matrix[row]; for (int column=0; column < columns; column++) {
         * r[column] = value++; //matrix[row][column] = value++; } }
         * timer1.stop(); } timer1.display(); timer1.minus(emptyLoop).display();
         * System.out.println(size / timer1.minus(emptyLoop).seconds() +"
         * elements / sec");
         * 
         * Runtime.getRuntime().gc(); // invite gc try {
         ...
         * 
         * System.out.println("\nNow reading..."); timer2.start(); int
         * element=0; for (int i=0; i<runs; i++) { for (int row=0; row < rows;
         * row++) { int[] r = matrix[row]; for (int column=0; column < columns;
         * column++) { element += r[column]; //element += matrix[row][column]; } } }
         * timer2.stop().display(); timer2.minus(emptyLoop2).display();
         * System.out.println(size / timer2.minus(emptyLoop2).seconds() +"
         * elements / sec"); if (print) { DenseIntMatrix2D m = new
         * DenseIntMatrix2D(rows,columns); m.assign(matrix);
         * System.out.println(m); } System.out.println(element); // !!! so that
         * the jitter can't optimize away the whole loop
         * 
         * System.out.println("\nNow removing..."); before =
         * Runtime.getRuntime().freeMemory(); for (int i=0; i<runs; i++) {
         * timer3.start(); for (int row=0; row < rows; row++) { int[] r =
         * matrix[row]; for (int column=0; column < columns; column++) {
         * r[column] = 0; //matrix[row][column] = 0; } } timer3.stop(); }
         * timer3.display(); timer3.minus(emptyLoop).display();
         * System.out.println(size / timer3.minus(emptyLoop).seconds() +"
         * elements / sec"); Runtime.getRuntime().gc(); // invite gc try {
         * Thread.currentThread().sleep(1000); } catch (InterruptedException
         * exc) {}; after = Runtime.getRuntime().freeMemory();
