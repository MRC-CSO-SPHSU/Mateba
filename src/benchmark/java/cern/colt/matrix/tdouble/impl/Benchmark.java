...
class Benchmark {
    ...
    public static void benchmark(int runs, int size, String kind, boolean print, int initialCapacity,
        ...
        if (kind.equals("sparse"))
            ...
            matrix = cern.colt.matrix.tdouble.DoubleFactory2D.dense.make(size, size);
        // else if (kind.equals("denseArray")) matrix = new
        // DoubleArrayMatrix2D(size,size);
        else
            throw new RuntimeException("unknown kind");

        System.out.println("\nNow initializing...");
        // Matrix AJ = new Matrix(columnwise,3);
        // Basic.random(matrix, new cern.jet.random.Uniform(new
        ...
        tmp = null;
        /*
         * long NN = matrix.size(); int nn = (int) (NN*percentNonZero); long[]
         * nonZeroIndexes = new long[nn]; cern.jet.random.sampling.RandomSampler
         * sampler = new cern.jet.random.sampling.RandomSampler(nn,NN,0,new
         * cern.jet.random.engine.MersenneTwister());
         * sampler.nextBlock(nn,nonZeroIndexes,0); for (int i=nn; --i >=0; ) {
         * int row = (int) (nonZeroIndexes[i]/size); int column = (int)
         * (nonZeroIndexes[i]%size); matrix.set(row,column, value); }
         */

        /*
         * timer1.start(); for (int i=0; i<runs; i++) { LUDecomposition LU =
         * new LUDecomposition(matrix); } timer1.stop(); timer1.display(); {
         * Jama.Matrix jmatrix = new Jama.Matrix(matrix.toArray());
         * timer2.start(); for (int i=0; i<runs; i++) { Jama.LUDecomposition LU =
         * new Jama.LUDecomposition(jmatrix); } timer2.stop(); timer2.display(); }
         */
        System.out.println("\ntesting...");
        if (print)
        ...
        System.out.println("\nNext testing...");
        /*
         * { timer6.start(); double a = cubicLoop(runs,size); timer6.stop();
         * timer6.display(); System.out.println(a); }
         */
        ...
        /*
         * { DoubleMatrix2D A = matrix.like().assign(value); DoubleMatrix2D B =
         * matrix.like().assign(value); DoubleMatrix2D C = Basic.product(A,B);
         * timer5.start(); for (int i=0; i<runs; i++) {
         * cern.colt.matrix.Blas.matrixMultiply(A,B,C); } timer5.stop();
         * timer5.display(); }
         */

        /*
         * { Jama.Matrix A = new Jama.Matrix(size,size); Jama.Matrix B = new
         * Jama.Matrix(size,size); Jama.Matrix C; timer4.start(); for (int i=0;
         * i<runs; i++) { C = A.times(B); } timer4.stop(); timer4.display(); }
         */
