...
class TestQuantileFinder {
    ...
    public static double epsilon(int size, double phi, double rank) {
        double s = size;
        // System.out.println("\n");
        // System.out.println("s="+size+", rank="+rank+", phi="+phi+",
        // eps="+Math.abs((rank)/s - phi));
        // System.out.println("\n");
                ...
                for (int e = 0; e < epsilons.length; e++) {
                    ...
                    for (int d = 0; d < deltas.length; d++) {
                        ...
                        for (int knownCounter = 0; knownCounter < 2; knownCounter++) {
                             ...
                             * long[] result; if (known_N) { result =
                             * QuantileFinderFactory.known_N_compute_B_and_K(N,
                             * epsilon, delta, p, returnSamplingRate); } else {
                             * result =
                            ...
                            if (mem == 0)
                                mem = N;
                            // else if (mem==0 && !known_N && N<0) mem =
                            // Long.MAX_VALUE; // actually infinity
                            ...
                            // System.out.print(")=("+(Math.round(b*k/1000.0))+","+b+","+k+",
    ...
    public static void testLocalVarDeclarationSpeed(int size) {
        ...
        /*
         * Timer timer = new Timer().start(); for (int i=0; i<size; i++) { for
         * (int j=0; j<size; j++) { DoubleBuffer buffer=null; int val=10;
         * double f=1.0f; } } System.out.println(timer.stop());
         */
    ...
    public static void testQuantileCalculation(String[] args) {
        ...
        /*
         * double[] returnSamplingRate = new double[1]; long[] result =
         * ApproximateQuantileFinder.computeBestBandK(size*chunks, epsilon,
         * delta, quantiles, returnSamplingRate); approxFinder = new
         * ApproximateQuantileFinder((int) result[0], (int) result[1]);
        ...
        // System.out.println("MaxLevel of full
        // buffers="+maxLevelOfFullBuffers(approxFinder.bufferSet));

        // System.out.println("total buffers filled="+
        // approxFinder.totalBuffersFilled);
        // System.out.println("free="+Runtime.getRuntime().freeMemory());
        // System.out.println("total="+Runtime.getRuntime().totalMemory());
        ...
        if (computeExactQuantilesAlso) {
            ...
            // double[] errors1 = errors1(exactQuantiles.elements(),
            // approxQuantiles.elements());
